...
class PlasticineModel:
    ...
    def get_router_spec(self, conf, vec):
        if conf['net'] == 'dynamic':
    ...
    def get_switch_spec(self, conf, vec):
        if vec:
    ...
    def get_area_summary(self, **conf):
        summary = OrderedDict()
        summary['router_unit_area'] = self.get_router_spec(conf, True)[0]
        summary['vswitch_unit_area'] = self.get_switch_spec(conf,True)[0]
        summary['sswitch_unit_area'] = self.get_switch_spec(conf,False)[0]
        summary['pcu_unit_area'] = self.get_pcu_area(conf)
        summary['pmu_unit_area'] = self.get_pmu_area(conf)
        summary['total_router_area'] = conf['nRouter'] * self.get_router_spec(conf, True)[0]
        summary['total_vswitch_area'] = conf['nSwitch'] * self.get_switch_spec(conf, True)[0]
        summary['total_sswitch_area'] = conf['nSwitch'] * self.get_switch_spec(conf, False)[0]
    ...
    def get_energy_summary(self, **conf):
        summary = OrderedDict()
        summary['router_flit_energy_scalar'] = self.get_router_spec(conf, False)[1]
        summary['router_flit_energy'] = self.get_router_spec(conf, True)[1]
        summary['vswitch_flit_energy'] = self.get_switch_spec(conf, True)[1]
        summary['sswitch_flit_energy'] = self.get_switch_spec(conf, False)[1]
    ...
    def get_power_summary(self, **conf):
        energy = self.get_energy_summary(**conf)
        power = OrderedDict()
        for k in energy: 
            if 'total' in k: power[k.replace('energy', 'power')] = energy[k] * conf['freq'] / conf['cycle']
        return power
