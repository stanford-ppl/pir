...
class PIRConfig(compiler:Compiler) extends spade.SpadeConfig(compiler) {
  ...
  register("retime-local", default=false, info="Enable retiming")
  register("retime-part", default=false, info="Enable retiming")
  ...
  def enablePartitionRetiming = option[Boolean]("retime-part")
