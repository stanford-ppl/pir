...
class SpatialPIRGenStaging(implicit compiler:PIRApp) extends PIRTransformer {
  ...
  def save[T](name:String, x:T) = { 
    ...
    x match {
      ...
      case n:ScanCounterDataFollower => analyzeCounterRange(n)
  ...
  def analyzeCounterRange(n:ScanCounter):Unit = dbgblk(s"analyzeCounterRange($n)") {
    val (constValids, constIters) = (List.tabulate(n.truePar) { i => if (n.truePar != n.par) Some(true) else None }, List.tabulate(n.truePar) { i => None })
    dbg(s"$n.constValids=[${constValids.map { _.getOrElse("unknown") }.mkString(",")}]")
    dbg(s"$n.constIters=[${constIters.map { _.getOrElse("unknown") }.mkString(",")}]")
    n.constValids := constValids
    n.constIters := constIters
  ...
  def analyzeCounterRange(n:ScanCounterDataFollower):Unit = dbgblk(s"analyzeCounterRange($n)") {
    val (constValids, constIters) = (List.tabulate(n.truePar) { i => Some(true) }, List.tabulate(n.truePar) { i => None })
    dbg(s"$n.constValids=[${constValids.map { _.getOrElse("unknown") }.mkString(",")}]")
    dbg(s"$n.constIters=[${constIters.map { _.getOrElse("unknown") }.mkString(",")}]")
    n.constValids := constValids
    n.constIters := constIters
