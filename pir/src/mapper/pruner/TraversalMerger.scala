...
trait TraversalMerger extends GlobalMerging with CSVPrinter with PartitionCost { self =>
  ...
  override def mergeGlobals(x:CUMap) = scheduler.map { scheduler =>
    val globs = pirTop.children.as[List[GlobalContainer]]
    val nodes = scheduler.scheduleScope(globs)
    x
  ...
  private def merge(nodes:List[GlobalContainer],x:CUMap, scheduler:GlobalScheduler):CUMap = {
    ...
    var canFit = true
    var (inpart, rest) = nodes.splitAt(2)
    while (canFit && rest.nonEmpty) {
      ...
      val vcosts = inpart.flatMap { k => x.freeValuesOf(k).filter(nextvs.contains) }.distinct
      canFit = if (vcosts.nonEmpty) {
        val vcost = vcosts.groupBy { _.param }.maxBy { _._2.size }._1
        fit(kcost, vcost)
      } else false
      if (!canFit) {
        rest = next :: rest
