...
object Differentiator extends PIRApp {
  def main(implicit design:PIRDesign) = {
    ...
    val x2142 = top.argFringe.argIn(init=0).name("x2142").ctrl(top) // ArgInNew(Const(0))
    ...
    val x2159 = StoreBanks(List(x2156_d0_b0), List(b1385), Const(0.0)).name("x2159").ctrl(x2160) // ParSRAMStore(x2156,List(List(b1385)),List(Const(0)),List(b1386))
    val x2161 = ReadMem(x2142).name("x2161").ctrl(x2262) // RegRead(x2142)
    val x2162 = Counter(min=Const(0), max=x2161, step=Const(64), par=1).name("x2162").ctrl(x2262) // CounterNew(Const(0),x2161,Const(64),Const(1))
    ...
    val x2164 = OpDef(op=FixAdd, inputs=List(b1392, Const(64))).name("x2164").ctrl(x2165) // FixAdd(b1392,Const(64))
    ...
    val x2167 = StreamIn(field="data").name("x2167").ctrl(x2185) // x2167 = StreamInNew(BurstDataBus())
    val x2176 = UnitController(style=SeqPipe, level=InnerControl).name("x2176").ctrl(x2185) // UnitPipe(List(b1393),Block(x2175))
    val x2168 = b1392 // FixConvert(b1392,TRUE,_32,_0)
    val x2169 = OpDef(op=FixSla, inputs=List(x2168, Const(2))).name("x2169").ctrl(x2176) // FixLsh(x2168,Const(2))
    val x2170 = x2169 // FixConvert(x2169,TRUE,_64,_0)
    val x2171 = top.argFringe.dramAddress(x2150).name("x2171").ctrl(x2176) // GetDRAMAddress(x2150)
    val x2172 = OpDef(op=FixAdd, inputs=List(x2170, x2171)).name("x2172").ctrl(x2176) // FixAdd(x2170,x2171)
    val x2174_x2173 = x2172 // FixConvert(x2172,TRUE,_64,_0)
    // x2174 = SimpleStruct(ArrayBuffer((offset,x2173), (size,Const(256)), (isLoad,Const(true))))
    val b2293_b2291 = WriteMem(b2291, x2174_x2173).name("b2293_b2291").ctrl(x2176) // StreamWrite(x2166,x2174,b1393)
    val b2294_b2292 = WriteMem(b2292, Const(256)).name("b2294_b2292").ctrl(x2176) // StreamWrite(x2166,x2174,b1393)
    val x2177 = FringeContainer(x2150,b2291,b2292,x2167).name("x2177").ctrl(x2185) // FringeDenseLoad(x2150,x2166,x2167)
    val x2178 = Counter(min=Const(0), max=Const(64), step=Const(1), par=1).name("x2178").ctrl(x2185) // CounterNew(Const(0),Const(64),Const(1),Const(1))
    ...
    val x2181_x2181 = ReadMem(x2167).name("x2181").ctrl(x2184) // ParStreamRead(x2167,List(x2180))
    val x2182_x2182 = x2181_x2181 // x2182 = VectorApply(x2181,0)
    val x2183 = StoreBanks(List(x2155_d0_b0), List(b1410), x2182_x2182).name("x2183").ctrl(x2184) // ParSRAMStore(x2155,List(List(b1410)),List(x2182),List(x2180))
    val x2186 = Counter(min=Const(0), max=Const(64), step=Const(1), par=1).name("x2186").ctrl(x2261) // CounterNew(Const(0),Const(64),Const(1),Const(1))
    ...
    val x2193 = Counter(min=Const(0), max=Const(8), step=Const(1), par=1).name("x2193").ctrl(x2236) // CounterNew(Const(0),Const(8),Const(1),Const(1))
    ...
    val x2200 = OpDef(op=FixEql, inputs=List(b1429, Const(0))).name("x2200").ctrl(x2204) // FixEql(b1429,Const(0))
    val x2201 = ReduceAccumOp(op=FixAdd, input=x2196, accum=x2199).name("x2201").ctrl(x2204) // FixAdd(x2196,x2199)
    val x2202 = OpDef(op=BitAnd, inputs=List(b1421, b1393)).name("x2202").ctrl(x2204) // And(b1421,b1393)
    val x2203_x2188_d0 = WriteMem(x2188_d0, x2201).name("x2203_x2188_d0").ctrl(x2204) // RegWrite(x2188,x2201,x2202)
    val x2203_x2188_d1 = WriteMem(x2188_d1, x2201).name("x2203_x2188_d1").ctrl(x2204) // RegWrite(x2188,x2201,x2202)
    ...
    val x2208 = OpDef(op=FixSra, inputs=List(x2207, Const(4))).name("x2208").ctrl(x2211) // FixRsh(x2207,Const(4))
    val x2209 = OpDef(op=BitAnd, inputs=List(b1421, b1393)).name("x2209").ctrl(x2211) // And(b1421,b1393)
    val x2210_x2205 = WriteMem(x2205, x2208).name("x2210_x2205").ctrl(x2211) // RegWrite(x2205,x2208,x2209)
    val x2212 = Counter(min=Const(0), max=Const(8), step=Const(1), par=1).name("x2212").ctrl(x2236) // CounterNew(Const(0),Const(8),Const(1),Const(1))
    ...
    val x2214 = OpDef(op=FixAdd, inputs=List(b1450, Const(8))).name("x2214").ctrl(x2224) // FixAdd(b1450,Const(8))
    ...
    val x2220 = OpDef(op=FixEql, inputs=List(b1450, Const(0))).name("x2220").ctrl(x2224) // FixEql(b1450,Const(0))
    val x2221 = ReduceAccumOp(op=FixAdd, input=x2216, accum=x2219).name("x2221").ctrl(x2224) // FixAdd(x2216,x2219)
    val x2222 = OpDef(op=BitAnd, inputs=List(b1421, b1393)).name("x2222").ctrl(x2224) // And(b1421,b1393)
    val x2223_x2206_d0 = WriteMem(x2206_d0, x2221).name("x2223_x2206_d0").ctrl(x2224) // RegWrite(x2206,x2221,x2222)
    val x2223_x2206_d1 = WriteMem(x2206_d1, x2221).name("x2223_x2206_d1").ctrl(x2224) // RegWrite(x2206,x2221,x2222)
    ...
    val x2226 = OpDef(op=FixSra, inputs=List(x2225, Const(4))).name("x2226").ctrl(x2235) // FixRsh(x2225,Const(4))
    ...
    val x2229 = OpDef(op=FixSra, inputs=List(x2228, Const(3))).name("x2229").ctrl(x2235) // FixRsh(x2228,Const(3))
    val x2230 = OpDef(op=FixAdd, inputs=List(b1420, b1392)).name("x2230").ctrl(x2235) // FixAdd(b1420,b1392)
    val x2231 = OpDef(op=FixLt, inputs=List(x2230, Const(16))).name("x2231").ctrl(x2235) // FixLt(x2230,Const(16))
    val x2232 = OpDef(op=MuxOp, inputs=List(x2231, Const(0.0), x2229)).name("x2232").ctrl(x2235) // Mux(x2231,Const(0),x2229)
    ...
    val b2295 = StreamOut(field="offset").name("b2295").ctrl(x2260) // x2237 = StreamOutNew(BurstCmdBus)
    val b2296 = StreamOut(field="size").name("b2296").ctrl(x2260) // x2237 = StreamOutNew(BurstCmdBus)
    val x2238 = StreamOut(field="data").name("x2238").ctrl(x2260) // x2238 = StreamOutNew(BurstFullDataBus())
    val x2239 = StreamIn(field="ack").name("x2239").ctrl(x2260) // x2239 = StreamInNew(BurstAckBus)
    val x2248 = UnitController(style=SeqPipe, level=InnerControl).name("x2248").ctrl(x2260) // UnitPipe(List(b1393),Block(x2247))
    val x2240 = b1392 // FixConvert(b1392,TRUE,_32,_0)
    val x2241 = OpDef(op=FixSla, inputs=List(x2240, Const(2))).name("x2241").ctrl(x2248) // FixLsh(x2240,Const(2))
    val x2242 = x2241 // FixConvert(x2241,TRUE,_64,_0)
    val x2243 = top.argFringe.dramAddress(x2153).name("x2243").ctrl(x2248) // GetDRAMAddress(x2153)
    val x2244 = OpDef(op=FixAdd, inputs=List(x2242, x2243)).name("x2244").ctrl(x2248) // FixAdd(x2242,x2243)
    val x2246_x2245 = x2244 // FixConvert(x2244,TRUE,_64,_0)
    // x2246 = SimpleStruct(ArrayBuffer((offset,x2245), (size,Const(256)), (isLoad,Const(false))))
    val b2297_b2295 = WriteMem(b2295, x2246_x2245).name("b2297_b2295").ctrl(x2248) // StreamWrite(x2237,x2246,b1393)
    val b2298_b2296 = WriteMem(b2296, Const(256)).name("b2298_b2296").ctrl(x2248) // StreamWrite(x2237,x2246,b1393)
    val x2249 = Counter(min=Const(0), max=Const(64), step=Const(1), par=1).name("x2249").ctrl(x2260) // CounterNew(Const(0),Const(64),Const(1),Const(1))
    ...
    val x2254_x2253 = x2252 // x2253 = VectorApply(x2252,0)
    // x2254 = SimpleStruct(ArrayBuffer((_1,x2253), (_2,Const(true))))
    val x2255_x2238 = WriteMem(x2238, x2254_x2253).name("x2255_x2238").ctrl(x2256) // ParStreamWrite(x2238,List(x2254),List(x2251))
    val x2257 = FringeContainer(x2153,b2295,b2296,x2238,x2239).name("x2257").ctrl(x2260) // FringeDenseStore(x2153,x2237,x2238,x2239)
    val x2259 = UnitController(style=SeqPipe, level=InnerControl).name("x2259").ctrl(x2260) // UnitPipe(List(b1393),Block(Const(())))
    val x2258_x2258 = ReadMem(x2239).name("x2258").ctrl(x2259) // StreamRead(x2239,b1393)
