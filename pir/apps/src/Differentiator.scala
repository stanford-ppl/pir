...
object Differentiator extends PIRApp {
  def main(top:Top) = {
    ...
    val x2149 = LoadMem(x2142, None).name("x2149").ctrl(top) // RegRead(x2142)
    val x2150 = DRAM().name("x2150").ctrl(top) // x2150 = DRAMNew(ArrayBuffer(x2149),Const(0))
    val x2152 = LoadMem(x2142, None).name("x2152").ctrl(top) // RegRead(x2142)
    ...
    val x2159 = StoreBanks(List(List(x2156_d0_b0)), List(b1385), Const(0)).name("x2159").ctrl(x2160) // ParSRAMStore(x2156,List(List(b1385)),List(Const(0)),List(b1386))
    val x2161 = LoadMem(x2142, None).name("x2161").ctrl(x2262) // RegRead(x2142)
    ...
    val b2294 = StoreMem(List(b2291), None, x2173).name("b2294").ctrl(x2176) // StreamWrite(x2166,x2174,b1393)
    val b2295 = StoreMem(List(b2292), None, Const(256)).name("b2295").ctrl(x2176) // StreamWrite(x2166,x2174,b1393)
    ...
    val b2296 = LoadMem(b2293, None).name("b2296").ctrl(x2184) // ParStreamRead(x2167,List(x2180))
    val b2297 = b2296 // x2182 = VectorApply(x2181,0)
    val x2183 = StoreBanks(List(List(x2155_d0_b0)), List(b1410), b2297).name("x2183").ctrl(x2184) // ParSRAMStore(x2155,List(List(b1410)),List(x2182),List(x2180))
    ...
    val x2191 = StoreMem(List(x2154_d0, x2154_d1), Some(List(Const(0), Const(0))), x2190).name("x2191").ctrl(x2192) // RegFileShiftIn(x2154,List(Const(0), Const(0)),1,x2190,x2189)
    ...
    val x2195 = LoadMem(x2154_d1, Some(List(Const(0), b1429))).name("x2195").ctrl(x2204) // ParRegFileLoad(x2154,List(List(Const(0), b1429)),List(Const(true)))
    ...
    val x2199 = LoadMem(x2188_d1, None).name("x2199").ctrl(x2204) // RegRead(x2188)
    ...
    val x2203 = StoreMem(List(x2188_d0, x2188_d1), None, x2201).name("x2203").ctrl(x2204) // RegWrite(x2188,x2201,x2202)
    ...
    val x2207 = LoadMem(x2188_d0, None).name("x2207").ctrl(x2211) // RegRead(x2188)
    ...
    val x2210 = StoreMem(List(x2205), None, x2208).name("x2210").ctrl(x2211) // RegWrite(x2205,x2208,x2209)
    ...
    val x2215 = LoadMem(x2154_d0, Some(List(Const(0), x2214))).name("x2215").ctrl(x2224) // ParRegFileLoad(x2154,List(List(Const(0), x2214)),List(Const(true)))
    ...
    val x2219 = LoadMem(x2206_d1, None).name("x2219").ctrl(x2224) // RegRead(x2206)
    ...
    val x2223 = StoreMem(List(x2206_d0, x2206_d1), None, x2221).name("x2223").ctrl(x2224) // RegWrite(x2206,x2221,x2222)
    val x2235 = UnitController(style=SeqPipe, level=InnerControl).name("x2235").ctrl(x2236) // UnitPipe(List(b1421, b1393),Block(Const(())))
    val x2225 = LoadMem(x2206_d0, None).name("x2225").ctrl(x2235) // RegRead(x2206)
    val x2226 = OpDef(op=FixSra, inputs=List(x2225, Const(4).ctrl(x2235))).name("x2226").ctrl(x2235) // FixRsh(x2225,Const(4))
    val x2227 = LoadMem(x2205, None).name("x2227").ctrl(x2235) // RegRead(x2205)
    ...
    val x2234 = StoreBanks(List(List(x2156_d0_b0)), List(b1420), x2232).name("x2234").ctrl(x2235) // SRAMStore(x2156,ArrayBuffer(Const(64)),List(b1420),Const(0),x2232,x2233)
    ...
    val b2302 = StoreMem(List(b2298), None, x2245).name("b2302").ctrl(x2248) // StreamWrite(x2237,x2246,b1393)
    val b2303 = StoreMem(List(b2299), None, Const(256)).name("b2303").ctrl(x2248) // StreamWrite(x2237,x2246,b1393)
    ...
    val b2304 = StoreMem(List(b2300), None, x2253).name("b2304").ctrl(x2256) // ParStreamWrite(x2238,List(x2254),List(x2251))
    ...
    val b2305 = LoadMem(b2301, None).name("b2305").ctrl(x2259) // StreamRead(x2239,b1393)
