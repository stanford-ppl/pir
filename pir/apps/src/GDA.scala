...
object GDA extends PIRApp {
  def main(top:Top) = {
    val x4038_d0 = top.argIn(init=0).name("x4038_d0").ctrl(top) // ArgInNew(Const(0))
    val x4040 = LoadDef(List(x4038_d0), None).name("x4040").ctrl(top) // RegRead(x4038)
    val x4041 = DRAM().name("x4041").ctrl(top) // x4041 = DRAMNew(ArrayBuffer(x4040, Const(96)),Const(0))
    val x4042 = LoadDef(List(x4038_d0), None).name("x4042").ctrl(top) // RegRead(x4038)
    ...
    val b4414 = StreamIn(field="data").name("b4414").ctrl(x4071) // x4054 = StreamInNew(BurstDataBus())
    val x4063 = UnitController(style=SeqPipe, level=InnerControl).name("x4063").ctrl(x4071) // UnitPipe(List(Const(true)),Block(x4062))
    val x4055 = OpDef(op=FixConvert, inputs=List(Const(0).ctrl(x4063))).name("x4055").ctrl(x4063) // FixConvert(Const(0),TRUE,_32,_0)
    val x4056 = OpDef(op=FixSla, inputs=List(x4055, Const(2).ctrl(x4063))).name("x4056").ctrl(x4063) // FixLsh(x4055,Const(2))
    val x4057 = OpDef(op=FixConvert, inputs=List(x4056)).name("x4057").ctrl(x4063) // FixConvert(x4056,TRUE,_64,_0)
    val x4058 = top.dramAddress(x4044).name("x4058").ctrl(x4063) // GetDRAMAddress(x4044)
    val x4059 = OpDef(op=FixAdd, inputs=List(x4057, x4058)).name("x4059").ctrl(x4063) // FixAdd(x4057,x4058)
    val x4060 = OpDef(op=FixConvert, inputs=List(x4059)).name("x4060").ctrl(x4063) // FixConvert(x4059,TRUE,_64,_0)
    // x4061 = SimpleStruct(ArrayBuffer((offset,x4060), (size,Const(384)), (isLoad,Const(true))))
    val b4415 = StoreDef(List(b4412), None, x4060).name("b4415").ctrl(x4063) // StreamWrite(x4053,x4061,Const(true))
    val b4416 = StoreDef(List(b4413), None, Const(384)).name("b4416").ctrl(x4063) // StreamWrite(x4053,x4061,Const(true))
    val x4064 = FringeContainer(x4044,b4412,b4413,b4414).name("x4064").ctrl(x4071) // FringeDenseLoad(x4044,x4053,x4054)
    ...
    val b2102 = IterDef(x4065, None).ctrl(x4070).name("b2102")
    val b2103 = DummyDef().ctrl(x4070).name("b2103")
    val b4417 = LoadDef(List(b4414), None).name("b4417").ctrl(x4070) // ParStreamRead(x4054,List(b2103))
    val b4418 = b4417 // x4068 = VectorApply(x4067,0)
    val x4069 = StoreDef(List(x4051_d0_b0, x4051_d1_b0), Some(List(b2102)), b4418).name("x4069").ctrl(x4070) // ParSRAMStore(x4051,List(List(b2102)),List(x4068),List(b2103))
    val x4090 = UnitController(style=StreamPipe, level=OuterControl).name("x4090").ctrl(x4360) // UnitPipe(List(Const(true)),Block(Const(())))
    val b4419 = StreamOut(field="offset").name("b4419").ctrl(x4090) // x4072 = StreamOutNew(BurstCmdBus)
    val b4420 = StreamOut(field="size").name("b4420").ctrl(x4090) // x4072 = StreamOutNew(BurstCmdBus)
    val b4421 = StreamIn(field="data").name("b4421").ctrl(x4090) // x4073 = StreamInNew(BurstDataBus())
    val x4082 = UnitController(style=SeqPipe, level=InnerControl).name("x4082").ctrl(x4090) // UnitPipe(List(Const(true)),Block(x4081))
    val x4074 = OpDef(op=FixConvert, inputs=List(Const(0).ctrl(x4082))).name("x4074").ctrl(x4082) // FixConvert(Const(0),TRUE,_32,_0)
    val x4075 = OpDef(op=FixSla, inputs=List(x4074, Const(2).ctrl(x4082))).name("x4075").ctrl(x4082) // FixLsh(x4074,Const(2))
    val x4076 = OpDef(op=FixConvert, inputs=List(x4075)).name("x4076").ctrl(x4082) // FixConvert(x4075,TRUE,_64,_0)
    val x4077 = top.dramAddress(x4045).name("x4077").ctrl(x4082) // GetDRAMAddress(x4045)
    val x4078 = OpDef(op=FixAdd, inputs=List(x4076, x4077)).name("x4078").ctrl(x4082) // FixAdd(x4076,x4077)
    val x4079 = OpDef(op=FixConvert, inputs=List(x4078)).name("x4079").ctrl(x4082) // FixConvert(x4078,TRUE,_64,_0)
    // x4080 = SimpleStruct(ArrayBuffer((offset,x4079), (size,Const(384)), (isLoad,Const(true))))
    val b4422 = StoreDef(List(b4419), None, x4079).name("b4422").ctrl(x4082) // StreamWrite(x4072,x4080,Const(true))
    val b4423 = StoreDef(List(b4420), None, Const(384)).name("b4423").ctrl(x4082) // StreamWrite(x4072,x4080,Const(true))
    val x4083 = FringeContainer(x4045,b4419,b4420,b4421).name("x4083").ctrl(x4090) // FringeDenseLoad(x4045,x4072,x4073)
    ...
    val b2123 = IterDef(x4084, None).ctrl(x4089).name("b2123")
    val b2124 = DummyDef().ctrl(x4089).name("b2124")
    val b4424 = LoadDef(List(b4421), None).name("b4424").ctrl(x4089) // ParStreamRead(x4073,List(b2124))
    val b4425 = b4424 // x4087 = VectorApply(x4086,0)
    val x4088 = StoreDef(List(x4052_d0_b0, x4052_d1_b0), Some(List(b2123)), b4425).name("x4088").ctrl(x4089) // ParSRAMStore(x4052,List(List(b2123)),List(x4087),List(b2124))
    ...
    val x4092 = LoadDef(List(x4038_d0), None).name("x4092").ctrl(x4360) // RegRead(x4038)
    ...
    val b2138 = IterDef(x4093, Some(0)).ctrl(x4330).name("b2138")
    val b2140 = DummyDef().ctrl(x4330).name("b2140")
    val b2139 = IterDef(x4093, Some(1)).ctrl(x4330).name("b2139")
    val b2141 = DummyDef().ctrl(x4330).name("b2141")
    ...
    val x4101 = OpDef(op=FixAdd, inputs=List(b2138, Const(20).ctrl(x4102))).name("x4101").ctrl(x4102) // FixAdd(b2138,Const(20))
    val x4122 = UnitController(style=StreamPipe, level=OuterControl).name("x4122").ctrl(x4330) // UnitPipe(List(b2140),Block(Const(())))
    val b4426 = StreamOut(field="offset").name("b4426").ctrl(x4122) // x4103 = StreamOutNew(BurstCmdBus)
    val b4427 = StreamOut(field="size").name("b4427").ctrl(x4122) // x4103 = StreamOutNew(BurstCmdBus)
    val b4428 = StreamIn(field="data").name("b4428").ctrl(x4122) // x4104 = StreamInNew(BurstDataBus())
    val x4113 = UnitController(style=SeqPipe, level=InnerControl).name("x4113").ctrl(x4122) // UnitPipe(List(b2140),Block(x4112))
    val x4105 = OpDef(op=FixConvert, inputs=List(b2138)).name("x4105").ctrl(x4113) // FixConvert(b2138,TRUE,_32,_0)
    val x4106 = OpDef(op=FixSla, inputs=List(x4105, Const(2).ctrl(x4113))).name("x4106").ctrl(x4113) // FixLsh(x4105,Const(2))
    val x4107 = OpDef(op=FixConvert, inputs=List(x4106)).name("x4107").ctrl(x4113) // FixConvert(x4106,TRUE,_64,_0)
    val x4108 = top.dramAddress(x4043).name("x4108").ctrl(x4113) // GetDRAMAddress(x4043)
    val x4109 = OpDef(op=FixAdd, inputs=List(x4107, x4108)).name("x4109").ctrl(x4113) // FixAdd(x4107,x4108)
    val x4110 = OpDef(op=FixConvert, inputs=List(x4109)).name("x4110").ctrl(x4113) // FixConvert(x4109,TRUE,_64,_0)
    // x4111 = SimpleStruct(ArrayBuffer((offset,x4110), (size,Const(80)), (isLoad,Const(true))))
    val b4429 = StoreDef(List(b4426), None, x4110).name("b4429").ctrl(x4113) // StreamWrite(x4103,x4111,b2140)
    val b4430 = StoreDef(List(b4427), None, Const(80)).name("b4430").ctrl(x4113) // StreamWrite(x4103,x4111,b2140)
    val x4114 = FringeContainer(x4043,b4426,b4427,b4428).name("x4114").ctrl(x4122) // FringeDenseLoad(x4043,x4103,x4104)
    ...
    val b2164 = IterDef(x4115, None).ctrl(x4121).name("b2164")
    val b2165 = DummyDef().ctrl(x4121).name("b2165")
    val x4117 = OpDef(op=BitAnd, inputs=List(b2165, b2140)).name("x4117").ctrl(x4121) // And(b2165,b2140)
    val b4431 = LoadDef(List(b4428), None).name("b4431").ctrl(x4121) // ParStreamRead(x4104,List(x4117))
    val b4432 = b4431 // x4119 = VectorApply(x4118,0)
    val x4120 = StoreDef(List(x4095_d0_b0), Some(List(b2164)), b4432).name("x4120").ctrl(x4121) // ParSRAMStore(x4095,List(List(b2164)),List(x4119),List(x4117))
    ...
    val b2174 = IterDef(x4123, Some(0)).ctrl(x4150).name("b2174")
    val b2175 = DummyDef().ctrl(x4150).name("b2175")
    val b4433 = StreamOut(field="offset").name("b4433").ctrl(x4150) // x4125 = StreamOutNew(BurstCmdBus)
    val b4434 = StreamOut(field="size").name("b4434").ctrl(x4150) // x4125 = StreamOutNew(BurstCmdBus)
    val b4435 = StreamIn(field="data").name("b4435").ctrl(x4150) // x4126 = StreamInNew(BurstDataBus())
    ...
    val x4128 = OpDef(op=FixConvert, inputs=List(x4127)).name("x4128").ctrl(x4140) // FixConvert(x4127,TRUE,_32,_0)
    val x4129 = OpDef(op=FixMul, inputs=List(x4128, Const(96).ctrl(x4140))).name("x4129").ctrl(x4140) // FixMul(x4128,Const(96))
    val x4130 = OpDef(op=FixConvert, inputs=List(Const(0).ctrl(x4140))).name("x4130").ctrl(x4140) // FixConvert(Const(0),TRUE,_32,_0)
    val x4131 = OpDef(op=FixAdd, inputs=List(x4129, x4130)).name("x4131").ctrl(x4140) // FixAdd(x4129,x4130)
    val x4132 = OpDef(op=FixSla, inputs=List(x4131, Const(2).ctrl(x4140))).name("x4132").ctrl(x4140) // FixLsh(x4131,Const(2))
    val x4133 = OpDef(op=FixConvert, inputs=List(x4132)).name("x4133").ctrl(x4140) // FixConvert(x4132,TRUE,_64,_0)
    val x4134 = top.dramAddress(x4041).name("x4134").ctrl(x4140) // GetDRAMAddress(x4041)
    val x4135 = OpDef(op=FixAdd, inputs=List(x4133, x4134)).name("x4135").ctrl(x4140) // FixAdd(x4133,x4134)
    val x4136 = OpDef(op=FixConvert, inputs=List(x4135)).name("x4136").ctrl(x4140) // FixConvert(x4135,TRUE,_64,_0)
    ...
    val b4436 = StoreDef(List(b4433), None, x4136).name("b4436").ctrl(x4140) // StreamWrite(x4125,x4137,x4138)
    val b4437 = StoreDef(List(b4434), None, Const(384)).name("b4437").ctrl(x4140) // StreamWrite(x4125,x4137,x4138)
    val x4141 = FringeContainer(x4041,b4433,b4434,b4435).name("x4141").ctrl(x4150) // FringeDenseLoad(x4041,x4125,x4126)
    ...
    val b2195 = IterDef(x4142, None).ctrl(x4149).name("b2195")
    val b2196 = DummyDef().ctrl(x4149).name("b2196")
    ...
    val b4438 = LoadDef(List(b4435), None).name("b4438").ctrl(x4149) // ParStreamRead(x4126,List(x4145))
    val b4439 = b4438 // x4147 = VectorApply(x4146,0)
    val x4148 = StoreDef(List(x4097_d0_b0), Some(List(b2174, b2195)), b4439).name("x4148").ctrl(x4149) // ParSRAMStore(x4097,List(List(b2174, b2195)),List(x4147),List(x4145))
    val x4155 = UnitController(style=SeqPipe, level=InnerControl).name("x4155").ctrl(x4330) // UnitPipe(List(b2140),Block(x4154))
    val x4151 = LoadDef(List(x4038_d0), None).name("x4151").ctrl(x4155) // RegRead(x4038)
    val x4152 = OpDef(op=FixSub, inputs=List(x4151, b2138)).name("x4152").ctrl(x4155) // FixSub(x4151,b2138)
    val x4153 = OpDef(op=FixMin, inputs=List(x4152, Const(20).ctrl(x4155))).name("x4153").ctrl(x4155) // Min(x4152,Const(20))
    val x4154 = StoreDef(List(x4099), None, x4153).name("x4154").ctrl(x4155) // RegWrite(x4099,x4153,b2140)
    val x4157 = UnitController(style=SeqPipe, level=InnerControl).name("x4157").ctrl(x4330) // UnitPipe(List(b2141),Block(Const(())))
    val x4156 = OpDef(op=FixAdd, inputs=List(b2139, Const(20).ctrl(x4157))).name("x4156").ctrl(x4157) // FixAdd(b2139,Const(20))
    val x4177 = UnitController(style=StreamPipe, level=OuterControl).name("x4177").ctrl(x4330) // UnitPipe(List(b2141),Block(Const(())))
    val b4440 = StreamOut(field="offset").name("b4440").ctrl(x4177) // x4158 = StreamOutNew(BurstCmdBus)
    val b4441 = StreamOut(field="size").name("b4441").ctrl(x4177) // x4158 = StreamOutNew(BurstCmdBus)
    val b4442 = StreamIn(field="data").name("b4442").ctrl(x4177) // x4159 = StreamInNew(BurstDataBus())
    val x4168 = UnitController(style=SeqPipe, level=InnerControl).name("x4168").ctrl(x4177) // UnitPipe(List(b2141),Block(x4167))
    val x4160 = OpDef(op=FixConvert, inputs=List(b2139)).name("x4160").ctrl(x4168) // FixConvert(b2139,TRUE,_32,_0)
    val x4161 = OpDef(op=FixSla, inputs=List(x4160, Const(2).ctrl(x4168))).name("x4161").ctrl(x4168) // FixLsh(x4160,Const(2))
    val x4162 = OpDef(op=FixConvert, inputs=List(x4161)).name("x4162").ctrl(x4168) // FixConvert(x4161,TRUE,_64,_0)
    val x4163 = top.dramAddress(x4043).name("x4163").ctrl(x4168) // GetDRAMAddress(x4043)
    val x4164 = OpDef(op=FixAdd, inputs=List(x4162, x4163)).name("x4164").ctrl(x4168) // FixAdd(x4162,x4163)
    val x4165 = OpDef(op=FixConvert, inputs=List(x4164)).name("x4165").ctrl(x4168) // FixConvert(x4164,TRUE,_64,_0)
    // x4166 = SimpleStruct(ArrayBuffer((offset,x4165), (size,Const(80)), (isLoad,Const(true))))
    val b4443 = StoreDef(List(b4440), None, x4165).name("b4443").ctrl(x4168) // StreamWrite(x4158,x4166,b2141)
    val b4444 = StoreDef(List(b4441), None, Const(80)).name("b4444").ctrl(x4168) // StreamWrite(x4158,x4166,b2141)
    val x4169 = FringeContainer(x4043,b4440,b4441,b4442).name("x4169").ctrl(x4177) // FringeDenseLoad(x4043,x4158,x4159)
    ...
    val b2226 = IterDef(x4170, None).ctrl(x4176).name("b2226")
    val b2227 = DummyDef().ctrl(x4176).name("b2227")
    val x4172 = OpDef(op=BitAnd, inputs=List(b2227, b2141)).name("x4172").ctrl(x4176) // And(b2227,b2141)
    val b4445 = LoadDef(List(b4442), None).name("b4445").ctrl(x4176) // ParStreamRead(x4159,List(x4172))
    val b4446 = b4445 // x4174 = VectorApply(x4173,0)
    val x4175 = StoreDef(List(x4096_d0_b0), Some(List(b2226)), b4446).name("x4175").ctrl(x4176) // ParSRAMStore(x4096,List(List(b2226)),List(x4174),List(x4172))
    ...
    val b2236 = IterDef(x4178, Some(0)).ctrl(x4205).name("b2236")
    val b2237 = DummyDef().ctrl(x4205).name("b2237")
    val b4447 = StreamOut(field="offset").name("b4447").ctrl(x4205) // x4180 = StreamOutNew(BurstCmdBus)
    val b4448 = StreamOut(field="size").name("b4448").ctrl(x4205) // x4180 = StreamOutNew(BurstCmdBus)
    val b4449 = StreamIn(field="data").name("b4449").ctrl(x4205) // x4181 = StreamInNew(BurstDataBus())
    ...
    val x4183 = OpDef(op=FixConvert, inputs=List(x4182)).name("x4183").ctrl(x4195) // FixConvert(x4182,TRUE,_32,_0)
    val x4184 = OpDef(op=FixMul, inputs=List(x4183, Const(96).ctrl(x4195))).name("x4184").ctrl(x4195) // FixMul(x4183,Const(96))
    val x4185 = OpDef(op=FixConvert, inputs=List(Const(0).ctrl(x4195))).name("x4185").ctrl(x4195) // FixConvert(Const(0),TRUE,_32,_0)
    val x4186 = OpDef(op=FixAdd, inputs=List(x4184, x4185)).name("x4186").ctrl(x4195) // FixAdd(x4184,x4185)
    val x4187 = OpDef(op=FixSla, inputs=List(x4186, Const(2).ctrl(x4195))).name("x4187").ctrl(x4195) // FixLsh(x4186,Const(2))
    val x4188 = OpDef(op=FixConvert, inputs=List(x4187)).name("x4188").ctrl(x4195) // FixConvert(x4187,TRUE,_64,_0)
    val x4189 = top.dramAddress(x4041).name("x4189").ctrl(x4195) // GetDRAMAddress(x4041)
    val x4190 = OpDef(op=FixAdd, inputs=List(x4188, x4189)).name("x4190").ctrl(x4195) // FixAdd(x4188,x4189)
    val x4191 = OpDef(op=FixConvert, inputs=List(x4190)).name("x4191").ctrl(x4195) // FixConvert(x4190,TRUE,_64,_0)
    ...
    val b4450 = StoreDef(List(b4447), None, x4191).name("b4450").ctrl(x4195) // StreamWrite(x4180,x4192,x4193)
    val b4451 = StoreDef(List(b4448), None, Const(384)).name("b4451").ctrl(x4195) // StreamWrite(x4180,x4192,x4193)
    val x4196 = FringeContainer(x4041,b4447,b4448,b4449).name("x4196").ctrl(x4205) // FringeDenseLoad(x4041,x4180,x4181)
    ...
    val b2257 = IterDef(x4197, None).ctrl(x4204).name("b2257")
    val b2258 = DummyDef().ctrl(x4204).name("b2258")
    ...
    val b4452 = LoadDef(List(b4449), None).name("b4452").ctrl(x4204) // ParStreamRead(x4181,List(x4200))
    val b4453 = b4452 // x4202 = VectorApply(x4201,0)
    val x4203 = StoreDef(List(x4098_d0_b0), Some(List(b2236, b2257)), b4453).name("x4203").ctrl(x4204) // ParSRAMStore(x4098,List(List(b2236, b2257)),List(x4202),List(x4200))
    val x4210 = UnitController(style=SeqPipe, level=InnerControl).name("x4210").ctrl(x4330) // UnitPipe(List(b2141),Block(x4209))
    val x4206 = LoadDef(List(x4038_d0), None).name("x4206").ctrl(x4210) // RegRead(x4038)
    val x4207 = OpDef(op=FixSub, inputs=List(x4206, b2139)).name("x4207").ctrl(x4210) // FixSub(x4206,b2139)
    val x4208 = OpDef(op=FixMin, inputs=List(x4207, Const(20).ctrl(x4210))).name("x4208").ctrl(x4210) // Min(x4207,Const(20))
    val x4209 = StoreDef(List(x4100), None, x4208).name("x4209").ctrl(x4210) // RegWrite(x4100,x4208,b2141)
    ...
    val x4213 = LoadDef(List(x4099), None).name("x4213").ctrl(x4330) // RegRead(x4099)
    ...
    val b2287 = IterDef(x4214, Some(0)).ctrl(x4261).name("b2287")
    val b2288 = DummyDef().ctrl(x4261).name("b2288")
    ...
    val b2293 = IterDef(x4218, None).ctrl(x4233).name("b2293")
    val b2294 = DummyDef().ctrl(x4233).name("b2294")
    ...
    val x4222 = LoadDef(List(x4097_d0_b0), Some(List(b2287, b2293))).name("x4222").ctrl(x4233) // ParSRAMLoad(x4097,List(List(b2287, b2293)),List(x4221))
    val x4223 = x4222 // x4223 = VectorApply(x4222,0)
    val x4224 = LoadDef(List(x4095_d0_b0), Some(List(b2287))).name("x4224").ctrl(x4233) // SRAMLoad(x4095,ArrayBuffer(Const(20)),List(b2287),Const(0),x4221)
    val x4225 = OpDef(op=FixEql, inputs=List(x4224, Const(1).ctrl(x4233))).name("x4225").ctrl(x4233) // FixEql(x4224,Const(1))
    val x4226 = LoadDef(List(x4052_d0_b0), Some(List(b2293))).name("x4226").ctrl(x4233) // ParSRAMLoad(x4052,List(List(b2293)),List(x4221))
    val x4227 = x4226 // x4227 = VectorApply(x4226,0)
    val x4228 = LoadDef(List(x4051_d0_b0), Some(List(b2293))).name("x4228").ctrl(x4233) // ParSRAMLoad(x4051,List(List(b2293)),List(x4221))
    ...
    val x4232 = StoreDef(List(x4216_d0_b0, x4216_d1_b0), Some(List(b2293)), x4231).name("x4232").ctrl(x4233) // ParSRAMStore(x4216,List(List(b2293)),List(x4231),List(x4221))
    ...
    val b2312 = IterDef(x4235, Some(0)).ctrl(x4245).name("b2312")
    val b2314 = DummyDef().ctrl(x4245).name("b2314")
    val b2313 = IterDef(x4234, None).ctrl(x4245).name("b2313")
    val b2315 = DummyDef().ctrl(x4245).name("b2315")
    ...
    val x4240 = LoadDef(List(x4216_d1_b0), Some(List(b2312))).name("x4240").ctrl(x4245) // SRAMLoad(x4216,ArrayBuffer(Const(96)),List(b2312),Const(0),x4239)
    val x4241 = LoadDef(List(x4216_d0_b0), Some(List(b2313))).name("x4241").ctrl(x4245) // ParSRAMLoad(x4216,List(List(b2313)),List(x4239))
    ...
    val x4244 = StoreDef(List(x4217_d0_b0), Some(List(b2312, b2313)), x4243).name("x4244").ctrl(x4245) // ParSRAMStore(x4217,List(List(b2312, b2313)),List(x4243),List(x4239))
    ...
    val b2325 = IterDef(x4247, Some(0)).ctrl(x4260).name("b2325")
    val b2327 = DummyDef().ctrl(x4260).name("b2327")
    val b2326 = IterDef(x4246, None).ctrl(x4260).name("b2326")
    val b2328 = DummyDef().ctrl(x4260).name("b2328")
    ...
    val x4251 = LoadDef(List(x4217_d0_b0), Some(List(b2325, b2326))).name("x4251").ctrl(x4260) // ParSRAMLoad(x4217,List(ArrayBuffer(b2325, b2326)),List(x4250))
    val x4252 = x4251 // x4252 = VectorApply(x4251,0)
    val x4253 = LoadDef(List(x4211_d1_b0), Some(List(b2325, b2326))).name("x4253").ctrl(x4260) // ParSRAMLoad(x4211,List(ArrayBuffer(b2325, b2326)),List(x4250))
    ...
    val x4257 = OpDef(op=FixEql, inputs=List(b2287, Const(0).ctrl(x4260))).name("x4257").ctrl(x4260) // FixEql(b2287,Const(0))
    val x4258 = OpDef(op=FixAdd, inputs=List(x4252, x4254)).name("x4258").ctrl(x4260) // FixAdd(x4252,x4254)
    val x4259 = StoreDef(List(x4211_d0_b0, x4211_d1_b0), Some(List(b2325, b2326)), x4258).name("x4259").ctrl(x4260) // ParSRAMStore(x4211,List(ArrayBuffer(b2325, b2326)),List(x4258),List(x4250))
    val x4262 = LoadDef(List(x4100), None).name("x4262").ctrl(x4330) // RegRead(x4100)
    ...
    val b2342 = IterDef(x4263, Some(0)).ctrl(x4310).name("b2342")
    val b2343 = DummyDef().ctrl(x4310).name("b2343")
    ...
    val b2348 = IterDef(x4267, None).ctrl(x4282).name("b2348")
    val b2349 = DummyDef().ctrl(x4282).name("b2349")
    ...
    val x4271 = LoadDef(List(x4098_d0_b0), Some(List(b2342, b2348))).name("x4271").ctrl(x4282) // ParSRAMLoad(x4098,List(List(b2342, b2348)),List(x4270))
    val x4272 = x4271 // x4272 = VectorApply(x4271,0)
    val x4273 = LoadDef(List(x4096_d0_b0), Some(List(b2342))).name("x4273").ctrl(x4282) // SRAMLoad(x4096,ArrayBuffer(Const(20)),List(b2342),Const(0),x4270)
    val x4274 = OpDef(op=FixEql, inputs=List(x4273, Const(1).ctrl(x4282))).name("x4274").ctrl(x4282) // FixEql(x4273,Const(1))
    val x4275 = LoadDef(List(x4052_d1_b0), Some(List(b2348))).name("x4275").ctrl(x4282) // ParSRAMLoad(x4052,List(List(b2348)),List(x4270))
    val x4276 = x4275 // x4276 = VectorApply(x4275,0)
    val x4277 = LoadDef(List(x4051_d1_b0), Some(List(b2348))).name("x4277").ctrl(x4282) // ParSRAMLoad(x4051,List(List(b2348)),List(x4270))
    ...
    val x4281 = StoreDef(List(x4265_d0_b0, x4265_d1_b0), Some(List(b2348)), x4280).name("x4281").ctrl(x4282) // ParSRAMStore(x4265,List(List(b2348)),List(x4280),List(x4270))
    ...
    val b2367 = IterDef(x4284, Some(0)).ctrl(x4294).name("b2367")
    val b2369 = DummyDef().ctrl(x4294).name("b2369")
    val b2368 = IterDef(x4283, None).ctrl(x4294).name("b2368")
    val b2370 = DummyDef().ctrl(x4294).name("b2370")
    ...
    val x4289 = LoadDef(List(x4265_d1_b0), Some(List(b2367))).name("x4289").ctrl(x4294) // SRAMLoad(x4265,ArrayBuffer(Const(96)),List(b2367),Const(0),x4288)
    val x4290 = LoadDef(List(x4265_d0_b0), Some(List(b2368))).name("x4290").ctrl(x4294) // ParSRAMLoad(x4265,List(List(b2368)),List(x4288))
    ...
    val x4293 = StoreDef(List(x4266_d0_b0), Some(List(b2367, b2368)), x4292).name("x4293").ctrl(x4294) // ParSRAMStore(x4266,List(List(b2367, b2368)),List(x4292),List(x4288))
    ...
    val b2380 = IterDef(x4296, Some(0)).ctrl(x4309).name("b2380")
    val b2382 = DummyDef().ctrl(x4309).name("b2382")
    val b2381 = IterDef(x4295, None).ctrl(x4309).name("b2381")
    val b2383 = DummyDef().ctrl(x4309).name("b2383")
    ...
    val x4300 = LoadDef(List(x4266_d0_b0), Some(List(b2380, b2381))).name("x4300").ctrl(x4309) // ParSRAMLoad(x4266,List(ArrayBuffer(b2380, b2381)),List(x4299))
    val x4301 = x4300 // x4301 = VectorApply(x4300,0)
    val x4302 = LoadDef(List(x4212_d1_b0), Some(List(b2380, b2381))).name("x4302").ctrl(x4309) // ParSRAMLoad(x4212,List(ArrayBuffer(b2380, b2381)),List(x4299))
    ...
    val x4306 = OpDef(op=FixEql, inputs=List(b2342, Const(0).ctrl(x4309))).name("x4306").ctrl(x4309) // FixEql(b2342,Const(0))
    val x4307 = OpDef(op=FixAdd, inputs=List(x4301, x4303)).name("x4307").ctrl(x4309) // FixAdd(x4301,x4303)
    val x4308 = StoreDef(List(x4212_d0_b0, x4212_d1_b0), Some(List(b2380, b2381)), x4307).name("x4308").ctrl(x4309) // ParSRAMStore(x4212,List(ArrayBuffer(b2380, b2381)),List(x4307),List(x4299))
    ...
    val b2398 = IterDef(x4312, Some(0)).ctrl(x4329).name("b2398")
    val b2400 = DummyDef().ctrl(x4329).name("b2400")
    val b2399 = IterDef(x4311, None).ctrl(x4329).name("b2399")
    val b2401 = DummyDef().ctrl(x4329).name("b2401")
    val x4314 = OpDef(op=BitAnd, inputs=List(b2400, b2401)).name("x4314").ctrl(x4329) // And(b2400,b2401)
    val x4315 = LoadDef(List(x4211_d0_b0), Some(List(b2398, b2399))).name("x4315").ctrl(x4329) // ParSRAMLoad(x4211,List(ArrayBuffer(b2398, b2399)),List(x4314))
    val x4316 = x4315 // x4316 = VectorApply(x4315,0)
    val x4317 = LoadDef(List(x4212_d0_b0), Some(List(b2398, b2399))).name("x4317").ctrl(x4329) // ParSRAMLoad(x4212,List(ArrayBuffer(b2398, b2399)),List(x4314))
    val x4318 = x4317 // x4318 = VectorApply(x4317,0)
    val x4319 = LoadDef(List(x4091_d1_b0), Some(List(b2398, b2399))).name("x4319").ctrl(x4329) // ParSRAMLoad(x4091,List(ArrayBuffer(b2398, b2399)),List(x4314))
    ...
    val x4326 = OpDef(op=FixEql, inputs=List(b2138, Const(0).ctrl(x4329))).name("x4326").ctrl(x4329) // FixEql(b2138,Const(0))
    val x4327 = OpDef(op=FixAdd, inputs=List(x4324, x4320)).name("x4327").ctrl(x4329) // FixAdd(x4324,x4320)
    val x4328 = StoreDef(List(x4091_d0_b0, x4091_d1_b0), Some(List(b2398, b2399)), x4327).name("x4328").ctrl(x4329) // ParSRAMStore(x4091,List(ArrayBuffer(b2398, b2399)),List(x4327),List(x4314))
    ...
    val b2421 = IterDef(x4331, Some(0)).ctrl(x4359).name("b2421")
    val b2422 = DummyDef().ctrl(x4359).name("b2422")
    val b4454 = StreamOut(field="offset").name("b4454").ctrl(x4359) // x4333 = StreamOutNew(BurstCmdBus)
    val b4455 = StreamOut(field="size").name("b4455").ctrl(x4359) // x4333 = StreamOutNew(BurstCmdBus)
    val b4456 = StreamOut(field="data").name("b4456").ctrl(x4359) // x4334 = StreamOutNew(BurstFullDataBus())
    val b4457 = StreamIn(field="ack").name("b4457").ctrl(x4359) // x4335 = StreamInNew(BurstAckBus)
    val x4347 = UnitController(style=SeqPipe, level=InnerControl).name("x4347").ctrl(x4359) // UnitPipe(List(b2422),Block(x4346))
    val x4336 = OpDef(op=FixConvert, inputs=List(b2421)).name("x4336").ctrl(x4347) // FixConvert(b2421,TRUE,_32,_0)
    val x4337 = OpDef(op=FixMul, inputs=List(x4336, Const(96).ctrl(x4347))).name("x4337").ctrl(x4347) // FixMul(x4336,Const(96))
    val x4338 = OpDef(op=FixConvert, inputs=List(Const(0).ctrl(x4347))).name("x4338").ctrl(x4347) // FixConvert(Const(0),TRUE,_32,_0)
    val x4339 = OpDef(op=FixAdd, inputs=List(x4337, x4338)).name("x4339").ctrl(x4347) // FixAdd(x4337,x4338)
    val x4340 = OpDef(op=FixSla, inputs=List(x4339, Const(2).ctrl(x4347))).name("x4340").ctrl(x4347) // FixLsh(x4339,Const(2))
    val x4341 = OpDef(op=FixConvert, inputs=List(x4340)).name("x4341").ctrl(x4347) // FixConvert(x4340,TRUE,_64,_0)
    val x4342 = top.dramAddress(x4046).name("x4342").ctrl(x4347) // GetDRAMAddress(x4046)
    val x4343 = OpDef(op=FixAdd, inputs=List(x4341, x4342)).name("x4343").ctrl(x4347) // FixAdd(x4341,x4342)
    val x4344 = OpDef(op=FixConvert, inputs=List(x4343)).name("x4344").ctrl(x4347) // FixConvert(x4343,TRUE,_64,_0)
    // x4345 = SimpleStruct(ArrayBuffer((offset,x4344), (size,Const(384)), (isLoad,Const(false))))
    val b4458 = StoreDef(List(b4454), None, x4344).name("b4458").ctrl(x4347) // StreamWrite(x4333,x4345,b2422)
    val b4459 = StoreDef(List(b4455), None, Const(384)).name("b4459").ctrl(x4347) // StreamWrite(x4333,x4345,b2422)
    ...
    val b2440 = IterDef(x4348, None).ctrl(x4355).name("b2440")
    val b2441 = DummyDef().ctrl(x4355).name("b2441")
    val x4350 = OpDef(op=BitAnd, inputs=List(b2441, b2422)).name("x4350").ctrl(x4355) // And(b2441,b2422)
    val x4351 = LoadDef(List(x4091_d0_b0), Some(List(b2421, b2440))).name("x4351").ctrl(x4355) // ParSRAMLoad(x4091,List(List(b2421, b2440)),List(x4350))
    val x4352 = x4351 // x4352 = VectorApply(x4351,0)
    // x4353 = SimpleStruct(ArrayBuffer((_1,x4352), (_2,Const(true))))
    val b4460 = StoreDef(List(b4456), None, x4352).name("b4460").ctrl(x4355) // ParStreamWrite(x4334,List(x4353),List(x4350))
    val x4356 = FringeContainer(x4046,b4454,b4455,b4456,b4457).name("x4356").ctrl(x4359) // FringeDenseStore(x4046,x4333,x4334,x4335)
    val x4358 = UnitController(style=SeqPipe, level=InnerControl).name("x4358").ctrl(x4359) // UnitPipe(List(b2422),Block(Const(())))
    val b4461 = LoadDef(List(b4457), None).name("b4461").ctrl(x4358) // StreamRead(x4335,b2422)
