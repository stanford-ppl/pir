...
object SPMV_CRS extends PIRApp {
  def main(implicit design:PIRDesign) = {
    ...
    val x5043 = Counter(min=Const(0), max=Const(1), step=Const(1), par=1).name("x5043").ctrl(x5434) // CounterNew(Const(0),Const(1),Const(1),Const(1))
    ...
    val x5050 = OpDef(op=FixMul, inputs=List(b2790, Const(495))).name("x5050").ctrl(x5057) // FixMul(b2790,Const(495))
    val x5051 = OpDef(op=FixAdd, inputs=List(b2790, Const(1))).name("x5051").ctrl(x5057) // FixAdd(b2790,Const(1))
    val x5052 = OpDef(op=FixMul, inputs=List(x5051, Const(495))).name("x5052").ctrl(x5057) // FixMul(x5051,Const(495))
    val x5053 = OpDef(op=FixSub, inputs=List(x5052, x5050)).name("x5053").ctrl(x5057) // FixSub(x5052,x5050)
    val x5054_x5047 = WriteMem(x5047, x5050).name("x5054_x5047").ctrl(x5057) // RegWrite(x5047,x5050,b2791)
    val x5055_x5048 = WriteMem(x5048, x5051).name("x5055_x5048").ctrl(x5057) // RegWrite(x5048,x5051,b2791)
    val x5056_x5049 = WriteMem(x5049, x5053).name("x5056_x5049").ctrl(x5057) // RegWrite(x5049,x5053,b2791)
    ...
    isAccum(b5475) = false
    val x5060 = StreamIn(field="data").name("x5060").ctrl(x5121) // x5060 = StreamInNew(BurstDataBus())
    ...
    val x5063 = OpDef(op=FixSla, inputs=List(x5062, Const(2))).name("x5063").ctrl(x5092) // FixLsh(x5062,Const(2))
    ...
    val x5068 = OpDef(op=FixSla, inputs=List(x5067, Const(2))).name("x5068").ctrl(x5092) // FixLsh(x5067,Const(2))
    ...
    val x5074 = OpDef(op=FixEql, inputs=List(x5073, Const(0))).name("x5074").ctrl(x5092) // FixEql(x5073,Const(0))
    val x5075 = OpDef(op=FixSub, inputs=List(Const(64), x5073)).name("x5075").ctrl(x5092) // FixSub(Const(64),x5073)
    val x5076 = OpDef(op=MuxOp, inputs=List(x5074, Const(0), x5075)).name("x5076").ctrl(x5092) // Mux(x5074,Const(0),x5075)
    val x5090_x5077 = OpDef(op=FixSra, inputs=List(x5066, Const(2))).name("x5077").ctrl(x5092) // FixRsh(x5066,Const(2))
    val x5078 = OpDef(op=FixSra, inputs=List(x5076, Const(2))).name("x5078").ctrl(x5092) // FixRsh(x5076,Const(2))
    val x5090_x5079 = OpDef(op=FixAdd, inputs=List(x5090_x5077, x5067)).name("x5079").ctrl(x5092) // FixAdd(x5077,x5067)
    val x5080 = OpDef(op=FixAdd, inputs=List(x5067, x5090_x5077)).name("x5080").ctrl(x5092) // FixAdd(x5067,x5077)
    val x5090_x5081 = OpDef(op=FixAdd, inputs=List(x5080, x5078)).name("x5081").ctrl(x5092) // FixAdd(x5080,x5078)
    val x5082 = OpDef(op=FixAdd, inputs=List(x5068, x5066)).name("x5082").ctrl(x5092) // FixAdd(x5068,x5066)
    val x5088_x5083 = OpDef(op=FixAdd, inputs=List(x5082, x5076)).name("x5083").ctrl(x5092) // FixAdd(x5082,x5076)
    val x5084 = x5069 // FixConvert(x5069,TRUE,_64,_0)
    val x5085 = top.argFringe.dramAddress(x5024).name("x5085").ctrl(x5092) // GetDRAMAddress(x5024)
    val x5086 = OpDef(op=FixAdd, inputs=List(x5084, x5085)).name("x5086").ctrl(x5092) // FixAdd(x5084,x5085)
    val x5088_x5087 = x5086 // FixConvert(x5086,TRUE,_64,_0)
    // x5088 = SimpleStruct(ArrayBuffer((offset,x5087), (size,x5083), (isLoad,Const(true))))
    val b5476_b5471 = WriteMem(b5471, x5088_x5087).name("b5476_b5471").ctrl(x5092) // StreamWrite(x5058,x5088,b2791)
    val b5477_b5472 = WriteMem(b5472, x5088_x5083).name("b5477_b5472").ctrl(x5092) // StreamWrite(x5058,x5088,b2791)
    // x5090 = SimpleStruct(ArrayBuffer((size,x5081), (start,x5077), (end,x5079)))
    val b5478_b5473 = WriteMem(b5473, x5090_x5081).name("b5478_b5473").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    val b5479_b5474 = WriteMem(b5474, x5090_x5077).name("b5479_b5474").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    val b5480_b5475 = WriteMem(b5475, x5090_x5079).name("b5480_b5475").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    val x5093 = FringeContainer(x5024,b5471,b5472,x5060).name("x5093").ctrl(x5121) // FringeDenseLoad(x5024,x5058,x5060)
    ...
    val x5097_b5481 = ReadMem(b5473).name("b5481").ctrl(x5104) // FIFODeq(x5059,b2791)
    val x5097_b5482 = ReadMem(b5474).name("b5482").ctrl(x5104) // FIFODeq(x5059,b2791)
    val x5097_b5483 = ReadMem(b5475).name("b5483").ctrl(x5104) // FIFODeq(x5059,b2791)
    val x5098 = x5097_b5482 // x5098 = FieldApply(x5097,start)
    val x5099_x5094 = WriteMem(x5094, x5098).name("x5099_x5094").ctrl(x5104) // RegWrite(x5094,x5098,b2791)
    val x5100 = x5097_b5483 // x5100 = FieldApply(x5097,end)
    val x5101_x5095 = WriteMem(x5095, x5100).name("x5101_x5095").ctrl(x5104) // RegWrite(x5095,x5100,b2791)
    val x5102 = x5097_b5481 // x5102 = FieldApply(x5097,size)
    val x5103_x5096 = WriteMem(x5096, x5102).name("x5103_x5096").ctrl(x5104) // RegWrite(x5096,x5102,b2791)
    val x5105 = ReadMem(x5096).name("x5105").ctrl(x5120) // RegRead(x5096)
    val x5106 = Counter(min=Const(0), max=x5105, step=Const(1), par=16).name("x5106").ctrl(x5120) // CounterNew(Const(0),x5105,Const(1),Const(16))
    ...
    val x5115_x5115 = ReadMem(x5060).name("x5115").ctrl(x5119) // ParStreamRead(x5060,List(x5114))
    val x5116_x5116 = x5115_x5115 // x5116 = VectorApply(x5115,0)
    val x5117 = OpDef(op=BitAnd, inputs=List(x5112, x5114)).name("x5117").ctrl(x5119) // And(x5112,x5114)
    val x5118 = StoreBanks(List(x5045_d0_b0, x5045_d1_b0), List(x5113), x5116_x5116).name("x5118").ctrl(x5119) // ParSRAMStore(x5045,List(List(x5113)),List(x5116),List(x5117))
    val x5122 = Counter(min=Const(0), max=Const(494), step=Const(1), par=1).name("x5122").ctrl(x5433) // CounterNew(Const(0),Const(494),Const(1),Const(1))
    ...
    val x5131 = OpDef(op=FixAdd, inputs=List(b2869, Const(1))).name("x5131").ctrl(x5135) // FixAdd(b2869,Const(1))
    ...
    val x5133_x5127_d0 = WriteMem(x5127_d0, x5130).name("x5133_x5127_d0").ctrl(x5135) // RegWrite(x5127,x5130,x5129)
    val x5133_x5127_d1 = WriteMem(x5127_d1, x5130).name("x5133_x5127_d1").ctrl(x5135) // RegWrite(x5127,x5130,x5129)
    val x5133_x5127_d2 = WriteMem(x5127_d2, x5130).name("x5133_x5127_d2").ctrl(x5135) // RegWrite(x5127,x5130,x5129)
    val x5133_x5127_d3 = WriteMem(x5127_d3, x5130).name("x5133_x5127_d3").ctrl(x5135) // RegWrite(x5127,x5130,x5129)
    val x5134_x5128_d0 = WriteMem(x5128_d0, x5132).name("x5134_x5128_d0").ctrl(x5135) // RegWrite(x5128,x5132,x5129)
    val x5134_x5128_d1 = WriteMem(x5128_d1, x5132).name("x5134_x5128_d1").ctrl(x5135) // RegWrite(x5128,x5132,x5129)
    val x5136_d0 = Reg(init=0).name("x5136_d0").ctrl(x5362) // x5136 = RegNew(Const(0))
    isAccum(x5136_d0) = false
    val x5136_d1 = Reg(init=0).name("x5136_d1").ctrl(x5362) // x5136 = RegNew(Const(0))
    isAccum(x5136_d1) = false
    ...
    val x5141_x5136_d0 = WriteMem(x5136_d0, x5139).name("x5141_x5136_d0").ctrl(x5142) // RegWrite(x5136,x5139,x5140)
    val x5141_x5136_d1 = WriteMem(x5136_d1, x5139).name("x5141_x5136_d1").ctrl(x5142) // RegWrite(x5136,x5139,x5140)
    val x5209 = UnitController(style=StreamPipe, level=OuterControl).name("x5209").ctrl(x5362) // UnitPipe(List(b2870, b2791),Block(Const(())))
    val b5484 = StreamOut(field="offset").name("b5484").ctrl(x5209) // x5143 = StreamOutNew(BurstCmdBus)
    val b5485 = StreamOut(field="size").name("b5485").ctrl(x5209) // x5143 = StreamOutNew(BurstCmdBus)
    val b5486 = FIFO(size=16).name("b5486").ctrl(x5209) // x5144 = FIFONew(Const(16))
    isAccum(b5486) = false
    val b5487 = FIFO(size=16).name("b5487").ctrl(x5209) // x5144 = FIFONew(Const(16))
    isAccum(b5487) = false
    val b5488 = FIFO(size=16).name("b5488").ctrl(x5209) // x5144 = FIFONew(Const(16))
    isAccum(b5488) = false
    val x5145 = StreamIn(field="data").name("x5145").ctrl(x5209) // x5145 = StreamInNew(BurstDataBus())
    ...
    val x5148 = OpDef(op=FixSla, inputs=List(x5147, Const(2))).name("x5148").ctrl(x5178) // FixLsh(x5147,Const(2))
    ...
    val x5153 = OpDef(op=FixSla, inputs=List(x5152, Const(2))).name("x5153").ctrl(x5178) // FixLsh(x5152,Const(2))
    ...
    val x5159 = OpDef(op=FixEql, inputs=List(x5158, Const(0))).name("x5159").ctrl(x5178) // FixEql(x5158,Const(0))
    val x5160 = OpDef(op=FixSub, inputs=List(Const(64), x5158)).name("x5160").ctrl(x5178) // FixSub(Const(64),x5158)
    val x5161 = OpDef(op=MuxOp, inputs=List(x5159, Const(0), x5160)).name("x5161").ctrl(x5178) // Mux(x5159,Const(0),x5160)
    val x5176_x5162 = OpDef(op=FixSra, inputs=List(x5151, Const(2))).name("x5162").ctrl(x5178) // FixRsh(x5151,Const(2))
    val x5163 = OpDef(op=FixSra, inputs=List(x5161, Const(2))).name("x5163").ctrl(x5178) // FixRsh(x5161,Const(2))
    val x5176_x5164 = OpDef(op=FixAdd, inputs=List(x5176_x5162, x5152)).name("x5164").ctrl(x5178) // FixAdd(x5162,x5152)
    val x5165 = OpDef(op=FixAdd, inputs=List(x5152, x5176_x5162)).name("x5165").ctrl(x5178) // FixAdd(x5152,x5162)
    val x5176_x5166 = OpDef(op=FixAdd, inputs=List(x5165, x5163)).name("x5166").ctrl(x5178) // FixAdd(x5165,x5163)
    val x5167 = OpDef(op=FixAdd, inputs=List(x5153, x5151)).name("x5167").ctrl(x5178) // FixAdd(x5153,x5151)
    val x5173_x5168 = OpDef(op=FixAdd, inputs=List(x5167, x5161)).name("x5168").ctrl(x5178) // FixAdd(x5167,x5161)
    val x5169 = x5154 // FixConvert(x5154,TRUE,_64,_0)
    val x5170 = top.argFringe.dramAddress(x5023).name("x5170").ctrl(x5178) // GetDRAMAddress(x5023)
    val x5171 = OpDef(op=FixAdd, inputs=List(x5169, x5170)).name("x5171").ctrl(x5178) // FixAdd(x5169,x5170)
    val x5173_x5172 = x5171 // FixConvert(x5171,TRUE,_64,_0)
    ...
    val b5489_b5484 = WriteMem(b5484, x5173_x5172).name("b5489_b5484").ctrl(x5178) // StreamWrite(x5143,x5173,x5174)
    val b5490_b5485 = WriteMem(b5485, x5173_x5168).name("b5490_b5485").ctrl(x5178) // StreamWrite(x5143,x5173,x5174)
    // x5176 = SimpleStruct(ArrayBuffer((size,x5166), (start,x5162), (end,x5164)))
    val b5491_b5486 = WriteMem(b5486, x5176_x5166).name("b5491_b5486").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    val b5492_b5487 = WriteMem(b5487, x5176_x5162).name("b5492_b5487").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    val b5493_b5488 = WriteMem(b5488, x5176_x5164).name("b5493_b5488").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    val x5179 = FringeContainer(x5023,b5484,b5485,x5145).name("x5179").ctrl(x5209) // FringeDenseLoad(x5023,x5143,x5145)
    ...
    val x5184_b5494 = ReadMem(b5486).name("b5494").ctrl(x5191) // FIFODeq(x5144,x5183)
    val x5184_b5495 = ReadMem(b5487).name("b5495").ctrl(x5191) // FIFODeq(x5144,x5183)
    val x5184_b5496 = ReadMem(b5488).name("b5496").ctrl(x5191) // FIFODeq(x5144,x5183)
    val x5185 = x5184_b5495 // x5185 = FieldApply(x5184,start)
    val x5186_x5180 = WriteMem(x5180, x5185).name("x5186_x5180").ctrl(x5191) // RegWrite(x5180,x5185,x5183)
    val x5187 = x5184_b5496 // x5187 = FieldApply(x5184,end)
    val x5188_x5181 = WriteMem(x5181, x5187).name("x5188_x5181").ctrl(x5191) // RegWrite(x5181,x5187,x5183)
    val x5189 = x5184_b5494 // x5189 = FieldApply(x5184,size)
    val x5190_x5182 = WriteMem(x5182, x5189).name("x5190_x5182").ctrl(x5191) // RegWrite(x5182,x5189,x5183)
    val x5192 = ReadMem(x5182).name("x5192").ctrl(x5208) // RegRead(x5182)
    val x5193 = Counter(min=Const(0), max=x5192, step=Const(1), par=1).name("x5193").ctrl(x5208) // CounterNew(Const(0),x5192,Const(1),Const(1))
    ...
    val x5203_x5203 = ReadMem(x5145).name("x5203").ctrl(x5207) // ParStreamRead(x5145,List(x5202))
    val x5204_x5204 = x5203_x5203 // x5204 = VectorApply(x5203,0)
    val x5205 = OpDef(op=BitAnd, inputs=List(x5199, x5202)).name("x5205").ctrl(x5207) // And(x5199,x5202)
    val x5206 = StoreBanks(List(x5124_d0_b0), List(x5200), x5204_x5204).name("x5206").ctrl(x5207) // ParSRAMStore(x5124,List(List(x5200)),List(x5204),List(x5205))
    val x5276 = UnitController(style=StreamPipe, level=OuterControl).name("x5276").ctrl(x5362) // UnitPipe(List(b2870, b2791),Block(Const(())))
    val b5497 = StreamOut(field="offset").name("b5497").ctrl(x5276) // x5210 = StreamOutNew(BurstCmdBus)
    val b5498 = StreamOut(field="size").name("b5498").ctrl(x5276) // x5210 = StreamOutNew(BurstCmdBus)
    val b5499 = FIFO(size=16).name("b5499").ctrl(x5276) // x5211 = FIFONew(Const(16))
    isAccum(b5499) = false
    val b5500 = FIFO(size=16).name("b5500").ctrl(x5276) // x5211 = FIFONew(Const(16))
    isAccum(b5500) = false
    val b5501 = FIFO(size=16).name("b5501").ctrl(x5276) // x5211 = FIFONew(Const(16))
    isAccum(b5501) = false
    val x5212 = StreamIn(field="data").name("x5212").ctrl(x5276) // x5212 = StreamInNew(BurstDataBus())
    ...
    val x5215 = OpDef(op=FixSla, inputs=List(x5214, Const(2))).name("x5215").ctrl(x5245) // FixLsh(x5214,Const(2))
    ...
    val x5220 = OpDef(op=FixSla, inputs=List(x5219, Const(2))).name("x5220").ctrl(x5245) // FixLsh(x5219,Const(2))
    ...
    val x5226 = OpDef(op=FixEql, inputs=List(x5225, Const(0))).name("x5226").ctrl(x5245) // FixEql(x5225,Const(0))
    val x5227 = OpDef(op=FixSub, inputs=List(Const(64), x5225)).name("x5227").ctrl(x5245) // FixSub(Const(64),x5225)
    val x5228 = OpDef(op=MuxOp, inputs=List(x5226, Const(0), x5227)).name("x5228").ctrl(x5245) // Mux(x5226,Const(0),x5227)
    val x5243_x5229 = OpDef(op=FixSra, inputs=List(x5218, Const(2))).name("x5229").ctrl(x5245) // FixRsh(x5218,Const(2))
    val x5230 = OpDef(op=FixSra, inputs=List(x5228, Const(2))).name("x5230").ctrl(x5245) // FixRsh(x5228,Const(2))
    val x5243_x5231 = OpDef(op=FixAdd, inputs=List(x5243_x5229, x5219)).name("x5231").ctrl(x5245) // FixAdd(x5229,x5219)
    val x5232 = OpDef(op=FixAdd, inputs=List(x5219, x5243_x5229)).name("x5232").ctrl(x5245) // FixAdd(x5219,x5229)
    val x5243_x5233 = OpDef(op=FixAdd, inputs=List(x5232, x5230)).name("x5233").ctrl(x5245) // FixAdd(x5232,x5230)
    val x5234 = OpDef(op=FixAdd, inputs=List(x5220, x5218)).name("x5234").ctrl(x5245) // FixAdd(x5220,x5218)
    val x5240_x5235 = OpDef(op=FixAdd, inputs=List(x5234, x5228)).name("x5235").ctrl(x5245) // FixAdd(x5234,x5228)
    val x5236 = x5221 // FixConvert(x5221,TRUE,_64,_0)
    val x5237 = top.argFringe.dramAddress(x5022).name("x5237").ctrl(x5245) // GetDRAMAddress(x5022)
    val x5238 = OpDef(op=FixAdd, inputs=List(x5236, x5237)).name("x5238").ctrl(x5245) // FixAdd(x5236,x5237)
    val x5240_x5239 = x5238 // FixConvert(x5238,TRUE,_64,_0)
    ...
    val b5502_b5497 = WriteMem(b5497, x5240_x5239).name("b5502_b5497").ctrl(x5245) // StreamWrite(x5210,x5240,x5241)
    val b5503_b5498 = WriteMem(b5498, x5240_x5235).name("b5503_b5498").ctrl(x5245) // StreamWrite(x5210,x5240,x5241)
    // x5243 = SimpleStruct(ArrayBuffer((size,x5233), (start,x5229), (end,x5231)))
    val b5504_b5499 = WriteMem(b5499, x5243_x5233).name("b5504_b5499").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    val b5505_b5500 = WriteMem(b5500, x5243_x5229).name("b5505_b5500").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    val b5506_b5501 = WriteMem(b5501, x5243_x5231).name("b5506_b5501").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    val x5246 = FringeContainer(x5022,b5497,b5498,x5212).name("x5246").ctrl(x5276) // FringeDenseLoad(x5022,x5210,x5212)
    ...
    val x5251_b5507 = ReadMem(b5499).name("b5507").ctrl(x5258) // FIFODeq(x5211,x5250)
    val x5251_b5508 = ReadMem(b5500).name("b5508").ctrl(x5258) // FIFODeq(x5211,x5250)
    val x5251_b5509 = ReadMem(b5501).name("b5509").ctrl(x5258) // FIFODeq(x5211,x5250)
    val x5252 = x5251_b5508 // x5252 = FieldApply(x5251,start)
    val x5253_x5247 = WriteMem(x5247, x5252).name("x5253_x5247").ctrl(x5258) // RegWrite(x5247,x5252,x5250)
    val x5254 = x5251_b5509 // x5254 = FieldApply(x5251,end)
    val x5255_x5248 = WriteMem(x5248, x5254).name("x5255_x5248").ctrl(x5258) // RegWrite(x5248,x5254,x5250)
    val x5256 = x5251_b5507 // x5256 = FieldApply(x5251,size)
    val x5257_x5249 = WriteMem(x5249, x5256).name("x5257_x5249").ctrl(x5258) // RegWrite(x5249,x5256,x5250)
    val x5259 = ReadMem(x5249).name("x5259").ctrl(x5275) // RegRead(x5249)
    val x5260 = Counter(min=Const(0), max=x5259, step=Const(1), par=1).name("x5260").ctrl(x5275) // CounterNew(Const(0),x5259,Const(1),Const(1))
    ...
    val x5282_x5277_d0 = WriteMem(x5277_d0, x5280).name("x5282_x5277_d0").ctrl(x5283) // RegWrite(x5277,x5280,x5281)
    val x5282_x5277_d1 = WriteMem(x5277_d1, x5280).name("x5282_x5277_d1").ctrl(x5283) // RegWrite(x5277,x5280,x5281)
    val x5282_x5277_d2 = WriteMem(x5277_d2, x5280).name("x5282_x5277_d2").ctrl(x5283) // RegWrite(x5277,x5280,x5281)
    val x5282_x5277_d3 = WriteMem(x5277_d3, x5280).name("x5282_x5277_d3").ctrl(x5283) // RegWrite(x5277,x5280,x5281)
    val x5284_d0 = Reg(init=0).name("x5284_d0").ctrl(x5362) // x5284 = RegNew(Const(0))
    isAccum(x5284_d0) = false
    val x5284_d1 = Reg(init=0).name("x5284_d1").ctrl(x5362) // x5284 = RegNew(Const(0))
    isAccum(x5284_d1) = false
    ...
    val x5286 = OpDef(op=FixLt, inputs=List(x5285, Const(16))).name("x5286").ctrl(x5297) // FixLt(x5285,Const(16))
    ...
    val x5290 = OpDef(op=FixEql, inputs=List(x5289, Const(0))).name("x5290").ctrl(x5297) // FixEql(x5289,Const(0))
    val x5291 = OpDef(op=FixAdd, inputs=List(x5285, Const(16))).name("x5291").ctrl(x5297) // FixAdd(x5285,Const(16))
    ...
    val x5294 = OpDef(op=MuxOp, inputs=List(x5286, Const(16), x5293)).name("x5294").ctrl(x5297) // Mux(x5286,Const(16),x5293)
    val x5295 = OpDef(op=BitAnd, inputs=List(b2870, b2791)).name("x5295").ctrl(x5297) // And(b2870,b2791)
    val x5296_x5284_d0 = WriteMem(x5284_d0, x5294).name("x5296_x5284_d0").ctrl(x5297) // RegWrite(x5284,x5294,x5295)
    val x5296_x5284_d1 = WriteMem(x5284_d1, x5294).name("x5296_x5284_d1").ctrl(x5297) // RegWrite(x5284,x5294,x5295)
    val x5330 = UnitController(style=StreamPipe, level=OuterControl).name("x5330").ctrl(x5362) // UnitPipe(List(b2870, b2791),Block(Const(())))
    val x5298 = StreamOut(field="addr").name("x5298").ctrl(x5330) // x5298 = StreamOutNew(GatherAddrBus)
    val x5299 = StreamIn(field="data").name("x5299").ctrl(x5330) // x5299 = StreamInNew(GatherDataBus())
    val x5300 = ReadMem(x5284_d1).name("x5300").ctrl(x5330) // RegRead(x5284)
    val x5301 = Counter(min=Const(0), max=x5300, step=Const(1), par=1).name("x5301").ctrl(x5330) // CounterNew(Const(0),x5300,Const(1),Const(1))
    ...
    val x5305 = top.argFringe.dramAddress(x5025).name("x5305").ctrl(x5316) // GetDRAMAddress(x5025)
    ...
    val x5311 = OpDef(op=FixSla, inputs=List(x5310, Const(2))).name("x5311").ctrl(x5316) // FixLsh(x5310,Const(2))
    ...
    val x5315_x5298 = WriteMem(x5298, x5314).name("x5315_x5298").ctrl(x5316) // ParStreamWrite(x5298,List(x5314),List(x5308))
    val x5317 = FringeContainer(x5025,x5298,x5299).name("x5317").ctrl(x5330) // FringeSparseLoad(x5025,x5298,x5299)
    val x5318 = ReadMem(x5284_d0).name("x5318").ctrl(x5330) // RegRead(x5284)
    val x5319 = Counter(min=Const(0), max=x5318, step=Const(1), par=1).name("x5319").ctrl(x5330) // CounterNew(Const(0),x5318,Const(1),Const(1))
    ...
    val x5323_x5323 = ReadMem(x5299).name("x5323").ctrl(x5329) // ParStreamRead(x5299,List(x5322))
    val x5324_x5324 = x5323_x5323 // x5324 = VectorApply(x5323,0)
    ...
    val x5328 = StoreBanks(List(x5126_d0_b0), List(b3065), x5324_x5324).name("x5328").ctrl(x5329) // ParSRAMStore(x5126,List(List(b3065)),List(x5324),List(x5327))
    val x5333_d0 = Reg(init=0.0).name("x5333_d0").ctrl(x5362) // x5333 = RegNew(Const(0))
    isAccum(x5333_d0) = false
    val x5333_d1 = Reg(init=0.0).name("x5333_d1").ctrl(x5362) // x5333 = RegNew(Const(0))
    isAccum(x5333_d1) = true
    ...
    val x5343 = Counter(min=Const(0), max=x5342, step=Const(1), par=2).name("x5343").ctrl(x5362) // CounterNew(Const(0),x5342,Const(1),Const(2))
    ...
    val x5353 = OpDef(op=FixEql, inputs=List(b3091, Const(0))).name("x5353").ctrl(x5357) // FixEql(b3091,Const(0))
    val x5354 = ReduceAccumOp(op=FixAdd, input=x5351, accum=x5352).name("x5354").ctrl(x5357) // FixAdd(x5351,x5352)
    val x5355 = OpDef(op=BitAnd, inputs=List(b2870, b2791)).name("x5355").ctrl(x5357) // And(b2870,b2791)
    val x5356_x5333_d0 = WriteMem(x5333_d0, x5354).name("x5356_x5333_d0").ctrl(x5357) // RegWrite(x5333,x5354,x5355)
    val x5356_x5333_d1 = WriteMem(x5333_d1, x5354).name("x5356_x5333_d1").ctrl(x5357) // RegWrite(x5333,x5354,x5355)
    ...
    val x5365 = OpDef(op=FixMul, inputs=List(b2790, Const(494))).name("x5365").ctrl(x5371) // FixMul(b2790,Const(494))
    val x5366 = ReadMem(x5048).name("x5366").ctrl(x5371) // RegRead(x5048)
    val x5367 = OpDef(op=FixMul, inputs=List(x5366, Const(494))).name("x5367").ctrl(x5371) // FixMul(x5366,Const(494))
    val x5368 = OpDef(op=FixSub, inputs=List(x5367, x5365)).name("x5368").ctrl(x5371) // FixSub(x5367,x5365)
    val x5369_x5363 = WriteMem(x5363, x5365).name("x5369_x5363").ctrl(x5371) // RegWrite(x5363,x5365,b2791)
    val x5370_x5364 = WriteMem(x5364, x5368).name("x5370_x5364").ctrl(x5371) // RegWrite(x5364,x5368,b2791)
    val x5432 = UnitController(style=StreamPipe, level=OuterControl).name("x5432").ctrl(x5433) // UnitPipe(List(b2791),Block(Const(())))
    val b5510 = StreamOut(field="offset").name("b5510").ctrl(x5432) // x5372 = StreamOutNew(BurstCmdBus)
    val b5511 = StreamOut(field="size").name("b5511").ctrl(x5432) // x5372 = StreamOutNew(BurstCmdBus)
    val x5373 = StreamOut(field="data").name("x5373").ctrl(x5432) // x5373 = StreamOutNew(BurstFullDataBus())
    val x5374 = StreamIn(field="ack").name("x5374").ctrl(x5432) // x5374 = StreamInNew(BurstAckBus)
    ...
    val x5380 = OpDef(op=FixSla, inputs=List(x5379, Const(2))).name("x5380").ctrl(x5410) // FixLsh(x5379,Const(2))
    ...
    val x5385 = OpDef(op=FixSla, inputs=List(x5384, Const(2))).name("x5385").ctrl(x5410) // FixLsh(x5384,Const(2))
    ...
    val x5391 = OpDef(op=FixEql, inputs=List(x5390, Const(0))).name("x5391").ctrl(x5410) // FixEql(x5390,Const(0))
    val x5392 = OpDef(op=FixSub, inputs=List(Const(64), x5390)).name("x5392").ctrl(x5410) // FixSub(Const(64),x5390)
    val x5393 = OpDef(op=MuxOp, inputs=List(x5391, Const(0), x5392)).name("x5393").ctrl(x5410) // Mux(x5391,Const(0),x5392)
    val x5394 = OpDef(op=FixSra, inputs=List(x5383, Const(2))).name("x5394").ctrl(x5410) // FixRsh(x5383,Const(2))
    val x5395 = OpDef(op=FixSra, inputs=List(x5393, Const(2))).name("x5395").ctrl(x5410) // FixRsh(x5393,Const(2))
    ...
    val x5405_x5400 = OpDef(op=FixAdd, inputs=List(x5399, x5393)).name("x5400").ctrl(x5410) // FixAdd(x5399,x5393)
    val x5401 = x5386 // FixConvert(x5386,TRUE,_64,_0)
    val x5402 = top.argFringe.dramAddress(x5026).name("x5402").ctrl(x5410) // GetDRAMAddress(x5026)
    val x5403 = OpDef(op=FixAdd, inputs=List(x5401, x5402)).name("x5403").ctrl(x5410) // FixAdd(x5401,x5402)
    val x5405_x5404 = x5403 // FixConvert(x5403,TRUE,_64,_0)
    // x5405 = SimpleStruct(ArrayBuffer((offset,x5404), (size,x5400), (isLoad,Const(false))))
    val b5512_b5510 = WriteMem(b5510, x5405_x5404).name("b5512_b5510").ctrl(x5410) // StreamWrite(x5372,x5405,b2791)
    val b5513_b5511 = WriteMem(b5511, x5405_x5400).name("b5513_b5511").ctrl(x5410) // StreamWrite(x5372,x5405,b2791)
    val x5407_x5375 = WriteMem(x5375, x5394).name("x5407_x5375").ctrl(x5410) // RegWrite(x5375,x5394,b2791)
    val x5408_x5376 = WriteMem(x5376, x5396).name("x5408_x5376").ctrl(x5410) // RegWrite(x5376,x5396,b2791)
    val x5409_x5377 = WriteMem(x5377, x5398).name("x5409_x5377").ctrl(x5410) // RegWrite(x5377,x5398,b2791)
    ...
    val x5425_x5418 = OpDef(op=BitAnd, inputs=List(x5415, x5417)).name("x5418").ctrl(x5427) // And(x5415,x5417)
    ...
    val x5421 = OpDef(op=BitAnd, inputs=List(x5425_x5418, x5420)).name("x5421").ctrl(x5427) // And(x5418,x5420)
    ...
    val x5425_x5424 = OpDef(op=MuxOp, inputs=List(x5425_x5418, x5423, Const(0.0))).name("x5424").ctrl(x5427) // Mux(x5418,x5423,Const(0))
    // x5425 = SimpleStruct(ArrayBuffer((_1,x5424), (_2,x5418)))
    val x5426_x5373 = WriteMem(x5373, x5425_x5424).name("x5426_x5373").ctrl(x5427) // ParStreamWrite(x5373,List(x5425),List(x5420))
    val x5429 = FringeContainer(x5026,b5510,b5511,x5373,x5374).name("x5429").ctrl(x5432) // FringeDenseStore(x5026,x5372,x5373,x5374)
    val x5431 = UnitController(style=SeqPipe, level=InnerControl).name("x5431").ctrl(x5432) // UnitPipe(List(b2791),Block(Const(())))
    val x5430_x5430 = ReadMem(x5374).name("x5430").ctrl(x5431) // StreamRead(x5374,b2791)
