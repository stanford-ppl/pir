...
object SPMV_CRS extends PIRApp {
  def main(top:Top) = {
    ...
    val b2790 = IterDef(x5043, Some(0)).ctrl(x5433).name("b2790")
    val b2791 = DummyDef().ctrl(x5433).name("b2791")
    ...
    val x5054 = StoreDef(List(x5047), None, x5050).name("x5054").ctrl(x5057) // RegWrite(x5047,x5050,b2791)
    val x5055 = StoreDef(List(x5048), None, x5051).name("x5055").ctrl(x5057) // RegWrite(x5048,x5051,b2791)
    val x5056 = StoreDef(List(x5049), None, x5053).name("x5056").ctrl(x5057) // RegWrite(x5049,x5053,b2791)
    ...
    val x5067 = LoadDef(List(x5049), None).name("x5067").ctrl(x5092) // RegRead(x5049)
    ...
    val b5479 = StoreDef(List(b5473), None, x5081).name("b5479").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    val b5480 = StoreDef(List(b5474), None, x5077).name("b5480").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    val b5481 = StoreDef(List(b5475), None, x5079).name("b5481").ctrl(x5092) // FIFOEnq(x5059,x5090,b2791)
    ...
    val b5482 = LoadDef(List(b5473), None).name("b5482").ctrl(x5104) // FIFODeq(x5059,b2791)
    val b5483 = LoadDef(List(b5474), None).name("b5483").ctrl(x5104) // FIFODeq(x5059,b2791)
    val b5484 = LoadDef(List(b5475), None).name("b5484").ctrl(x5104) // FIFODeq(x5059,b2791)
    val x5098 = b5483 // x5098 = FieldApply(x5097,start)
    val x5099 = StoreDef(List(x5094), None, x5098).name("x5099").ctrl(x5104) // RegWrite(x5094,x5098,b2791)
    val x5100 = b5484 // x5100 = FieldApply(x5097,end)
    val x5101 = StoreDef(List(x5095), None, x5100).name("x5101").ctrl(x5104) // RegWrite(x5095,x5100,b2791)
    val x5102 = b5482 // x5102 = FieldApply(x5097,size)
    val x5103 = StoreDef(List(x5096), None, x5102).name("x5103").ctrl(x5104) // RegWrite(x5096,x5102,b2791)
    val x5105 = LoadDef(List(x5096), None).name("x5105").ctrl(x5120) // RegRead(x5096)
    ...
    val b2851 = IterDef(x5106, None).ctrl(x5119).name("b2851")
    val b2852 = DummyDef().ctrl(x5119).name("b2852")
    val x5108 = LoadDef(List(x5094), None).name("x5108").ctrl(x5119) // RegRead(x5094)
    val x5109 = OpDef(op=FixLeq, inputs=List(x5108, b2851)).name("x5109").ctrl(x5119) // FixLeq(x5108,b2851)
    val x5110 = LoadDef(List(x5095), None).name("x5110").ctrl(x5119) // RegRead(x5095)
    ...
    val b5485 = LoadDef(List(b5476), None).name("b5485").ctrl(x5119) // ParStreamRead(x5060,List(x5114))
    ...
    val x5118 = StoreDef(List(x5045_d0_b0, x5045_d1_b0), Some(List(x5113)), b5486).name("x5118").ctrl(x5119) // ParSRAMStore(x5045,List(List(x5113)),List(x5116),List(x5117))
    ...
    val b2869 = IterDef(x5122, Some(0)).ctrl(x5362).name("b2869")
    val b2870 = DummyDef().ctrl(x5362).name("b2870")
    ...
    val x5130 = LoadDef(List(x5045_d1_b0), Some(List(b2869))).name("x5130").ctrl(x5135) // SRAMLoad(x5045,ArrayBuffer(Const(495)),List(b2869),Const(0),x5129)
    val x5131 = OpDef(op=FixAdd, inputs=List(b2869, Const(1).ctrl(x5135))).name("x5131").ctrl(x5135) // FixAdd(b2869,Const(1))
    val x5132 = LoadDef(List(x5045_d0_b0), Some(List(x5131))).name("x5132").ctrl(x5135) // SRAMLoad(x5045,ArrayBuffer(Const(495)),List(x5131),Const(0),x5129)
    val x5133 = StoreDef(List(x5127_d0, x5127_d1, x5127_d2, x5127_d3), None, x5130).name("x5133").ctrl(x5135) // RegWrite(x5127,x5130,x5129)
    val x5134 = StoreDef(List(x5128_d0, x5128_d1), None, x5132).name("x5134").ctrl(x5135) // RegWrite(x5128,x5132,x5129)
    ...
    val x5137 = LoadDef(List(x5128_d1), None).name("x5137").ctrl(x5142) // RegRead(x5128)
    val x5138 = LoadDef(List(x5127_d3), None).name("x5138").ctrl(x5142) // RegRead(x5127)
    ...
    val x5141 = StoreDef(List(x5136_d0, x5136_d1), None, x5139).name("x5141").ctrl(x5142) // RegWrite(x5136,x5139,x5140)
    ...
    val x5152 = LoadDef(List(x5136_d1), None).name("x5152").ctrl(x5178) // RegRead(x5136)
    ...
    val b5494 = StoreDef(List(b5488), None, x5168).name("b5494").ctrl(x5178) // StreamWrite(x5143,x5173,x5174)
    // x5176 = SimpleStruct(ArrayBuffer((size,x5166), (start,x5162), (end,x5164)))
    val b5495 = StoreDef(List(b5489), None, x5166).name("b5495").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    val b5496 = StoreDef(List(b5490), None, x5162).name("b5496").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    val b5497 = StoreDef(List(b5491), None, x5164).name("b5497").ctrl(x5178) // FIFOEnq(x5144,x5176,x5174)
    ...
    val b5498 = LoadDef(List(b5489), None).name("b5498").ctrl(x5191) // FIFODeq(x5144,x5183)
    val b5499 = LoadDef(List(b5490), None).name("b5499").ctrl(x5191) // FIFODeq(x5144,x5183)
    val b5500 = LoadDef(List(b5491), None).name("b5500").ctrl(x5191) // FIFODeq(x5144,x5183)
    val x5185 = b5499 // x5185 = FieldApply(x5184,start)
    val x5186 = StoreDef(List(x5180), None, x5185).name("x5186").ctrl(x5191) // RegWrite(x5180,x5185,x5183)
    val x5187 = b5500 // x5187 = FieldApply(x5184,end)
    val x5188 = StoreDef(List(x5181), None, x5187).name("x5188").ctrl(x5191) // RegWrite(x5181,x5187,x5183)
    val x5189 = b5498 // x5189 = FieldApply(x5184,size)
    val x5190 = StoreDef(List(x5182), None, x5189).name("x5190").ctrl(x5191) // RegWrite(x5182,x5189,x5183)
    val x5192 = LoadDef(List(x5182), None).name("x5192").ctrl(x5208) // RegRead(x5182)
    ...
    val b2938 = IterDef(x5193, None).ctrl(x5207).name("b2938")
    val b2939 = DummyDef().ctrl(x5207).name("b2939")
    val x5195 = LoadDef(List(x5180), None).name("x5195").ctrl(x5207) // RegRead(x5180)
    val x5196 = OpDef(op=FixLeq, inputs=List(x5195, b2938)).name("x5196").ctrl(x5207) // FixLeq(x5195,b2938)
    val x5197 = LoadDef(List(x5181), None).name("x5197").ctrl(x5207) // RegRead(x5181)
    ...
    val b5501 = LoadDef(List(b5492), None).name("b5501").ctrl(x5207) // ParStreamRead(x5145,List(x5202))
    ...
    val x5206 = StoreDef(List(x5124_d0_b0), Some(List(x5200)), b5502).name("x5206").ctrl(x5207) // ParSRAMStore(x5124,List(List(x5200)),List(x5204),List(x5205))
    ...
    val x5219 = LoadDef(List(x5136_d0), None).name("x5219").ctrl(x5245) // RegRead(x5136)
    ...
    val b5510 = StoreDef(List(b5504), None, x5235).name("b5510").ctrl(x5245) // StreamWrite(x5210,x5240,x5241)
    // x5243 = SimpleStruct(ArrayBuffer((size,x5233), (start,x5229), (end,x5231)))
    val b5511 = StoreDef(List(b5505), None, x5233).name("b5511").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    val b5512 = StoreDef(List(b5506), None, x5229).name("b5512").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    val b5513 = StoreDef(List(b5507), None, x5231).name("b5513").ctrl(x5245) // FIFOEnq(x5211,x5243,x5241)
    ...
    val b5514 = LoadDef(List(b5505), None).name("b5514").ctrl(x5258) // FIFODeq(x5211,x5250)
    val b5515 = LoadDef(List(b5506), None).name("b5515").ctrl(x5258) // FIFODeq(x5211,x5250)
    val b5516 = LoadDef(List(b5507), None).name("b5516").ctrl(x5258) // FIFODeq(x5211,x5250)
    val x5252 = b5515 // x5252 = FieldApply(x5251,start)
    val x5253 = StoreDef(List(x5247), None, x5252).name("x5253").ctrl(x5258) // RegWrite(x5247,x5252,x5250)
    val x5254 = b5516 // x5254 = FieldApply(x5251,end)
    val x5255 = StoreDef(List(x5248), None, x5254).name("x5255").ctrl(x5258) // RegWrite(x5248,x5254,x5250)
    val x5256 = b5514 // x5256 = FieldApply(x5251,size)
    val x5257 = StoreDef(List(x5249), None, x5256).name("x5257").ctrl(x5258) // RegWrite(x5249,x5256,x5250)
    val x5259 = LoadDef(List(x5249), None).name("x5259").ctrl(x5275) // RegRead(x5249)
    ...
    val b3003 = IterDef(x5260, None).ctrl(x5274).name("b3003")
    val b3004 = DummyDef().ctrl(x5274).name("b3004")
    val x5262 = LoadDef(List(x5247), None).name("x5262").ctrl(x5274) // RegRead(x5247)
    val x5263 = OpDef(op=FixLeq, inputs=List(x5262, b3003)).name("x5263").ctrl(x5274) // FixLeq(x5262,b3003)
    val x5264 = LoadDef(List(x5248), None).name("x5264").ctrl(x5274) // RegRead(x5248)
    ...
    val b5517 = LoadDef(List(b5508), None).name("b5517").ctrl(x5274) // ParStreamRead(x5212,List(x5269))
    ...
    val x5273 = StoreDef(List(x5125_d0_b0), Some(List(x5267)), b5518).name("x5273").ctrl(x5274) // ParSRAMStore(x5125,List(List(x5267)),List(x5271),List(x5272))
    ...
    val x5278 = LoadDef(List(x5128_d0), None).name("x5278").ctrl(x5283) // RegRead(x5128)
    val x5279 = LoadDef(List(x5127_d0), None).name("x5279").ctrl(x5283) // RegRead(x5127)
    ...
    val x5282 = StoreDef(List(x5277_d0, x5277_d1, x5277_d2, x5277_d3), None, x5280).name("x5282").ctrl(x5283) // RegWrite(x5277,x5280,x5281)
    ...
    val x5285 = LoadDef(List(x5277_d3), None).name("x5285").ctrl(x5297) // RegRead(x5277)
    ...
    val x5296 = StoreDef(List(x5284_d0, x5284_d1), None, x5294).name("x5296").ctrl(x5297) // RegWrite(x5284,x5294,x5295)
    ...
    val x5300 = LoadDef(List(x5284_d1), None).name("x5300").ctrl(x5330) // RegRead(x5284)
    ...
    val b3045 = IterDef(x5301, None).ctrl(x5316).name("b3045")
    val b3046 = DummyDef().ctrl(x5316).name("b3046")
    ...
    val x5318 = LoadDef(List(x5284_d0), None).name("x5318").ctrl(x5330) // RegRead(x5284)
    ...
    val b3065 = IterDef(x5319, None).ctrl(x5329).name("b3065")
    val b3066 = DummyDef().ctrl(x5329).name("b3066")
    ...
    val b5522 = LoadDef(List(b5520), None).name("b5522").ctrl(x5329) // ParStreamRead(x5299,List(x5322))
    val b5523 = b5522 // x5324 = VectorApply(x5323,0)
    val x5325 = LoadDef(List(x5277_d1), None).name("x5325").ctrl(x5329) // RegRead(x5277)
    ...
    val b3091 = IterDef(x5343, None).ctrl(x5357).name("b3091")
    val b3092 = DummyDef().ctrl(x5357).name("b3092")
    ...
    val x5347 = LoadDef(List(x5125_d0_b0), Some(List(b3091))).name("x5347").ctrl(x5357) // ParSRAMLoad(x5125,List(List(b3091)),List(x5346))
    val x5348 = x5347 // x5348 = VectorApply(x5347,0)
    val x5349 = LoadDef(List(x5126_d0_b0), Some(List(b3091))).name("x5349").ctrl(x5357) // ParSRAMLoad(x5126,List(List(b3091)),List(x5346))
    ...
    val x5352 = LoadDef(List(x5333_d1), None).name("x5352").ctrl(x5357) // RegRead(x5333)
    ...
    val x5356 = StoreDef(List(x5333_d0, x5333_d1), None, x5354).name("x5356").ctrl(x5357) // RegWrite(x5333,x5354,x5355)
    val x5361 = UnitController(style=SeqPipe, level=InnerControl).name("x5361").ctrl(x5362) // UnitPipe(List(b2870, b2791),Block(Const(())))
    val x5358 = LoadDef(List(x5333_d0), None).name("x5358").ctrl(x5361) // RegRead(x5333)
    val x5359 = OpDef(op=BitAnd, inputs=List(b2870, b2791)).name("x5359").ctrl(x5361) // And(b2870,b2791)
    val x5360 = StoreDef(List(x5046_d0_b0), Some(List(b2869)), x5358).name("x5360").ctrl(x5361) // SRAMStore(x5046,ArrayBuffer(Const(494)),List(b2869),Const(0),x5358,x5359)
    ...
    val x5366 = LoadDef(List(x5048), None).name("x5366").ctrl(x5371) // RegRead(x5048)
    ...
    val x5369 = StoreDef(List(x5363), None, x5365).name("x5369").ctrl(x5371) // RegWrite(x5363,x5365,b2791)
    val x5370 = StoreDef(List(x5364), None, x5368).name("x5370").ctrl(x5371) // RegWrite(x5364,x5368,b2791)
    ...
    val x5384 = LoadDef(List(x5364), None).name("x5384").ctrl(x5410) // RegRead(x5364)
    ...
    val x5407 = StoreDef(List(x5375), None, x5394).name("x5407").ctrl(x5410) // RegWrite(x5375,x5394,b2791)
    val x5408 = StoreDef(List(x5376), None, x5396).name("x5408").ctrl(x5410) // RegWrite(x5376,x5396,b2791)
    val x5409 = StoreDef(List(x5377), None, x5398).name("x5409").ctrl(x5410) // RegWrite(x5377,x5398,b2791)
    val x5411 = LoadDef(List(x5377), None).name("x5411").ctrl(x5428) // RegRead(x5377)
    ...
    val b3158 = IterDef(x5412, None).ctrl(x5427).name("b3158")
    val b3159 = DummyDef().ctrl(x5427).name("b3159")
    val x5414 = LoadDef(List(x5375), None).name("x5414").ctrl(x5427) // RegRead(x5375)
    val x5415 = OpDef(op=FixLeq, inputs=List(x5414, b3158)).name("x5415").ctrl(x5427) // FixLeq(x5414,b3158)
    val x5416 = LoadDef(List(x5376), None).name("x5416").ctrl(x5427) // RegRead(x5376)
    ...
    val x5422 = LoadDef(List(x5046_d0_b0), Some(List(x5419))).name("x5422").ctrl(x5427) // ParSRAMLoad(x5046,List(List(x5419)),List(x5421))
    ...
    val b5530 = StoreDef(List(b5526), None, x5424).name("b5530").ctrl(x5427) // ParStreamWrite(x5373,List(x5425),List(x5420))
