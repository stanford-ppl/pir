...
object BFS extends PIRApp {
  def main(top:Top) = {
    ...
    val x3260 = LoadDef(List(x3250), None).name("x3260").ctrl(x3483) // RegRead(x3250)
    ...
    val b1937 = IterDef(x3261, Some(0)).ctrl(x3268).name("b1937")
    val b1938 = DummyDef().ctrl(x3268).name("b1938")
    val x3267 = UnitController(style=SeqPipe, level=InnerControl).name("x3267").ctrl(x3268) // UnitPipe(List(b1938),Block(Const(())))
    val x3263 = StoreDef(List(x3252_d0_b0), Some(List(b1937)), Const(0)).name("x3263").ctrl(x3267) // SRAMStore(x3252,ArrayBuffer(Const(8000)),List(b1937),Const(0),Const(0),b1938)
    val x3264 = StoreDef(List(x3255_d0_b0, x3255_d1_b0), Some(List(b1937)), Const(0)).name("x3264").ctrl(x3267) // SRAMStore(x3255,ArrayBuffer(Const(8000)),List(b1937),Const(0),Const(0),b1938)
    val x3265 = StoreDef(List(x3256_d0_b0, x3256_d1_b0, x3256_d2_b0), Some(List(b1937)), Const(0)).name("x3265").ctrl(x3267) // SRAMStore(x3256,ArrayBuffer(Const(8000)),List(b1937),Const(0),Const(0),b1938)
    val x3266 = StoreDef(List(x3257_d0_b0), Some(List(b1937)), Const(0)).name("x3266").ctrl(x3267) // SRAMStore(x3257,ArrayBuffer(Const(8000)),List(b1937),Const(0),Const(0),b1938)
    ...
    val b3504 = StoreDef(List(b3501), None, x3276).name("b3504").ctrl(x3279) // StreamWrite(x3269,x3277,Const(true))
    val b3505 = StoreDef(List(b3502), None, Const(32000)).name("b3505").ctrl(x3279) // StreamWrite(x3269,x3277,Const(true))
    ...
    val b1960 = IterDef(x3281, None).ctrl(x3286).name("b1960")
    val b1961 = DummyDef().ctrl(x3286).name("b1961")
    val b3506 = LoadDef(List(b3503), None).name("b3506").ctrl(x3286) // ParStreamRead(x3270,List(b1961))
    val b3507 = b3506 // x3284 = VectorApply(x3283,0)
    val x3285 = StoreDef(List(x3254_d0_b0), Some(List(b1960)), b3507).name("x3285").ctrl(x3286) // ParSRAMStore(x3254,List(List(b1960)),List(x3284),List(b1961))
    ...
    val b3511 = StoreDef(List(b3508), None, x3295).name("b3511").ctrl(x3298) // StreamWrite(x3288,x3296,Const(true))
    val b3512 = StoreDef(List(b3509), None, Const(32000)).name("b3512").ctrl(x3298) // StreamWrite(x3288,x3296,Const(true))
    ...
    val b1981 = IterDef(x3300, None).ctrl(x3305).name("b1981")
    val b1982 = DummyDef().ctrl(x3305).name("b1982")
    val b3513 = LoadDef(List(b3510), None).name("b3513").ctrl(x3305) // ParStreamRead(x3289,List(b1982))
    val b3514 = b3513 // x3303 = VectorApply(x3302,0)
    val x3304 = StoreDef(List(x3253_d0_b0), Some(List(b1981)), b3514).name("x3304").ctrl(x3305) // ParSRAMStore(x3253,List(List(b1981)),List(x3303),List(b1982))
    val x3307 = LoadDef(List(x3248), None).name("x3307").ctrl(x3483) // RegRead(x3248)
    ...
    val b1992 = IterDef(x3308, Some(0)).ctrl(x3482).name("b1992")
    val b1993 = DummyDef().ctrl(x3482).name("b1993")
    ...
    val x3312 = LoadDef(List(x3259), None).name("x3312").ctrl(x3482) // RegRead(x3259)
    ...
    val b1999 = IterDef(x3313, Some(0)).ctrl(x3412).name("b1999")
    val b2000 = DummyDef().ctrl(x3412).name("b2000")
    ...
    val x3317 = LoadDef(List(x3256_d2_b0), Some(List(b1999))).name("x3317").ctrl(x3326) // SRAMLoad(x3256,ArrayBuffer(Const(8000)),List(b1999),Const(0),x3316)
    val x3318 = LoadDef(List(x3254_d0_b0), Some(List(x3317))).name("x3318").ctrl(x3326) // SRAMLoad(x3254,ArrayBuffer(Const(8000)),List(x3317),Const(0),x3316)
    val x3319 = StoreDef(List(x3311_d0, x3311_d1), None, x3318).name("x3319").ctrl(x3326) // RegWrite(x3311,x3318,x3316)
    val x3320 = LoadDef(List(x3253_d0_b0), Some(List(x3317))).name("x3320").ctrl(x3326) // SRAMLoad(x3253,ArrayBuffer(Const(8000)),List(x3317),Const(0),x3316)
    val x3321 = StoreDef(List(x3310_d0, x3310_d1, x3310_d2), None, x3320).name("x3321").ctrl(x3326) // RegWrite(x3310,x3320,x3316)
    val x3322 = LoadDef(List(x3311_d1), None).name("x3322").ctrl(x3326) // RegRead(x3311)
    val x3323 = LoadDef(List(x3310_d2), None).name("x3323").ctrl(x3326) // RegRead(x3310)
    val x3324 = OpDef(op=FixAdd, inputs=List(x3322, x3323)).name("x3324").ctrl(x3326) // FixAdd(x3322,x3323)
    val x3325 = StoreDef(List(x3315), None, x3323).name("x3325").ctrl(x3326) // RegWrite(x3315,x3323,x3316)
    ...
    val x3330 = LoadDef(List(x3311_d0), None).name("x3330").ctrl(x3362) // RegRead(x3311)
    ...
    val x3336 = LoadDef(List(x3315), None).name("x3336").ctrl(x3362) // RegRead(x3315)
    ...
    val b3521 = StoreDef(List(b3515), None, x3356).name("b3521").ctrl(x3362) // StreamWrite(x3327,x3357,x3358)
    val b3522 = StoreDef(List(b3516), None, x3352).name("b3522").ctrl(x3362) // StreamWrite(x3327,x3357,x3358)
    // x3360 = SimpleStruct(ArrayBuffer((size,x3350), (start,x3346), (end,x3348)))
    val b3523 = StoreDef(List(b3517), None, x3350).name("b3523").ctrl(x3362) // FIFOEnq(x3328,x3360,x3358)
    val b3524 = StoreDef(List(b3518), None, x3346).name("b3524").ctrl(x3362) // FIFOEnq(x3328,x3360,x3358)
    val b3525 = StoreDef(List(b3519), None, x3348).name("b3525").ctrl(x3362) // FIFOEnq(x3328,x3360,x3358)
    ...
    val b3526 = LoadDef(List(b3517), None).name("b3526").ctrl(x3375) // FIFODeq(x3328,x3367)
    val b3527 = LoadDef(List(b3518), None).name("b3527").ctrl(x3375) // FIFODeq(x3328,x3367)
    val b3528 = LoadDef(List(b3519), None).name("b3528").ctrl(x3375) // FIFODeq(x3328,x3367)
    val x3369 = b3527 // x3369 = FieldApply(x3368,start)
    val x3370 = StoreDef(List(x3364), None, x3369).name("x3370").ctrl(x3375) // RegWrite(x3364,x3369,x3367)
    val x3371 = b3528 // x3371 = FieldApply(x3368,end)
    val x3372 = StoreDef(List(x3365), None, x3371).name("x3372").ctrl(x3375) // RegWrite(x3365,x3371,x3367)
    val x3373 = b3526 // x3373 = FieldApply(x3368,size)
    val x3374 = StoreDef(List(x3366), None, x3373).name("x3374").ctrl(x3375) // RegWrite(x3366,x3373,x3367)
    val x3376 = LoadDef(List(x3366), None).name("x3376").ctrl(x3392) // RegRead(x3366)
    ...
    val b2061 = IterDef(x3377, None).ctrl(x3391).name("b2061")
    val b2062 = DummyDef().ctrl(x3391).name("b2062")
    val x3379 = LoadDef(List(x3364), None).name("x3379").ctrl(x3391) // RegRead(x3364)
    val x3380 = OpDef(op=FixLeq, inputs=List(x3379, b2061)).name("x3380").ctrl(x3391) // FixLeq(x3379,b2061)
    val x3381 = LoadDef(List(x3365), None).name("x3381").ctrl(x3391) // RegRead(x3365)
    ...
    val b3529 = LoadDef(List(b3520), None).name("b3529").ctrl(x3391) // ParStreamRead(x3329,List(x3386))
    ...
    val x3390 = StoreDef(List(x3257_d0_b0), Some(List(x3384)), b3530).name("x3390").ctrl(x3391) // ParSRAMStore(x3257,List(List(x3384)),List(x3388),List(x3389))
    val x3394 = LoadDef(List(x3310_d1), None).name("x3394").ctrl(x3412) // RegRead(x3310)
    ...
    val b2081 = IterDef(x3395, None).ctrl(x3404).name("b2081")
    val b2082 = DummyDef().ctrl(x3404).name("b2082")
    val x3397 = LoadDef(List(x3258_d6), None).name("x3397").ctrl(x3404) // RegRead(x3258)
    ...
    val x3401 = LoadDef(List(x3257_d0_b0), Some(List(b2081))).name("x3401").ctrl(x3404) // ParSRAMLoad(x3257,List(List(b2081)),List(x3400))
    val x3402 = x3401 // x3402 = VectorApply(x3401,0)
    val x3403 = StoreDef(List(x3252_d0_b0), Some(List(x3398)), x3402).name("x3403").ctrl(x3404) // ParSRAMStore(x3252,List(List(x3398)),List(x3402),List(x3400))
    val x3411 = UnitController(style=SeqPipe, level=InnerControl).name("x3411").ctrl(x3412) // UnitPipe(List(b2000, b1993),Block(Const(())))
    val x3405 = LoadDef(List(x3310_d0), None).name("x3405").ctrl(x3411) // RegRead(x3310)
    val x3406 = LoadDef(List(x3258_d5), None).name("x3406").ctrl(x3411) // RegRead(x3258)
    ...
    val x3410 = StoreDef(List(x3258_d0, x3258_d5, x3258_d1, x3258_d6, x3258_d2, x3258_d3, x3258_d4), None, x3408).name("x3410").ctrl(x3411) // RegWrite(x3258,x3408,x3409)
    val x3413 = LoadDef(List(x3258_d4), None).name("x3413").ctrl(x3482) // RegRead(x3258)
    ...
    val b2102 = IterDef(x3414, None).ctrl(x3420).name("b2102")
    val b2103 = DummyDef().ctrl(x3420).name("b2103")
    val x3416 = OpDef(op=BitAnd, inputs=List(b2103, b1993)).name("x3416").ctrl(x3420) // And(b2103,b1993)
    val x3417 = LoadDef(List(x3252_d0_b0), Some(List(b2102))).name("x3417").ctrl(x3420) // ParSRAMLoad(x3252,List(List(b2102)),List(x3416))
    val x3418 = x3417 // x3418 = VectorApply(x3417,0)
    val x3419 = StoreDef(List(x3256_d0_b0, x3256_d1_b0, x3256_d2_b0), Some(List(b2102)), x3418).name("x3419").ctrl(x3420) // ParSRAMStore(x3256,List(List(b2102)),List(x3418),List(x3416))
    val x3421 = LoadDef(List(x3258_d3), None).name("x3421").ctrl(x3482) // RegRead(x3258)
    ...
    val b2112 = IterDef(x3422, None).ctrl(x3427).name("b2112")
    val b2113 = DummyDef().ctrl(x3427).name("b2113")
    ...
    val x3426 = StoreDef(List(x3255_d0_b0, x3255_d1_b0), Some(List(b2112)), x3424).name("x3426").ctrl(x3427) // ParSRAMStore(x3255,List(List(b2112)),List(x3424),List(x3425))
    ...
    val x3429 = LoadDef(List(x3258_d2), None).name("x3429").ctrl(x3440) // RegRead(x3258)
    ...
    val x3439 = StoreDef(List(x3428_d0, x3428_d1), None, x3438).name("x3439").ctrl(x3440) // RegWrite(x3428,x3438,b1993)
    ...
    val x3443 = LoadDef(List(x3428_d1), None).name("x3443").ctrl(x3477) // RegRead(x3428)
    ...
    val b2134 = IterDef(x3444, None).ctrl(x3468).name("b2134")
    val b2135 = DummyDef().ctrl(x3468).name("b2135")
    val x3446 = LoadDef(List(x3258_d1), None).name("x3446").ctrl(x3468) // RegRead(x3258)
    ...
    val x3450 = LoadDef(List(x3256_d1_b0), Some(List(x3448))).name("x3450").ctrl(x3468) // SRAMLoad(x3256,ArrayBuffer(Const(8000)),List(x3448),Const(0),x3449)
    ...
    val x3456 = LoadDef(List(x3256_d0_b0), Some(List(b2134))).name("x3456").ctrl(x3468) // ParSRAMLoad(x3256,List(List(b2134)),List(x3449))
    ...
    val x3462 = LoadDef(List(x3255_d1_b0), Some(List(x3448))).name("x3462").ctrl(x3468) // SRAMLoad(x3255,ArrayBuffer(Const(8000)),List(x3448),Const(0),x3449)
    val x3463 = LoadDef(List(x3255_d0_b0), Some(List(b2134))).name("x3463").ctrl(x3468) // ParSRAMLoad(x3255,List(List(b2134)),List(x3449))
    ...
    val b3533 = StoreDef(List(b3531), None, x3465).name("b3533").ctrl(x3468) // ParStreamWrite(x3441,List(x3466),List(x3449))
    val x3469 = FringeContainer(x3244,b3531,b3532).name("x3469").ctrl(x3477) // FringeSparseStore(x3244,x3441,x3442)
    val x3470 = LoadDef(List(x3428_d0), None).name("x3470").ctrl(x3477) // RegRead(x3428)
    ...
    val b2163 = IterDef(x3471, None).ctrl(x3476).name("b2163")
    val b2164 = DummyDef().ctrl(x3476).name("b2164")
    val x3473 = OpDef(op=BitAnd, inputs=List(b2164, b1993)).name("x3473").ctrl(x3476) // And(b2164,b1993)
    val b3534 = LoadDef(List(b3532), None).name("b3534").ctrl(x3476) // ParStreamRead(x3442,List(x3473))
    ...
    val x3478 = LoadDef(List(x3258_d0), None).name("x3478").ctrl(x3481) // RegRead(x3258)
    val x3479 = StoreDef(List(x3259), None, x3478).name("x3479").ctrl(x3481) // RegWrite(x3259,x3478,b1993)
    val x3480 = StoreDef(List(x3258_d0, x3258_d5, x3258_d1, x3258_d6, x3258_d2, x3258_d3, x3258_d4), None, Const(0)).name("x3480").ctrl(x3481) // RegWrite(x3258,Const(0),b1993)
