...
object Gibbs_Ising2D extends PIRApp {
  def main(implicit design:PIRDesign) = {
    ...
    val x5090 = top.argFringe.argIn(init=0).name("x5090").ctrl(top) // ArgInNew(Const(0))
    val x5091_d0 = top.argFringe.argIn(init=0.0).name("x5091_d0").ctrl(top) // ArgInNew(Const(0))
    val x5092_d0 = top.argFringe.argIn(init=0.0).name("x5092_d0").ctrl(top) // ArgInNew(Const(0))
    ...
    val x5111 = StreamIn(field="data").name("x5111").ctrl(x5128) // x5111 = StreamInNew(BurstDataBus())
    val x5120 = UnitController(style=SeqPipe, level=InnerControl).name("x5120").ctrl(x5128) // UnitPipe(List(Const(true)),Block(x5119))
    val x5112 = Const(0) // FixConvert(Const(0),TRUE,_32,_0)
    val x5113 = OpDef(op=FixSla, inputs=List(x5112, Const(2))).name("x5113").ctrl(x5120) // FixLsh(x5112,Const(2))
    val x5114 = x5113 // FixConvert(x5113,TRUE,_64,_0)
    val x5115 = top.argFringe.dramAddress(x5099).name("x5115").ctrl(x5120) // GetDRAMAddress(x5099)
    val x5116 = OpDef(op=FixAdd, inputs=List(x5114, x5115)).name("x5116").ctrl(x5120) // FixAdd(x5114,x5115)
    val x5118_x5117 = x5116 // FixConvert(x5116,TRUE,_64,_0)
    // x5118 = SimpleStruct(ArrayBuffer((offset,x5117), (size,Const(36)), (isLoad,Const(true))))
    val b5623_b5621 = WriteMem(b5621, x5118_x5117).name("b5623_b5621").ctrl(x5120) // StreamWrite(x5110,x5118,Const(true))
    val b5624_b5622 = WriteMem(b5622, Const(36)).name("b5624_b5622").ctrl(x5120) // StreamWrite(x5110,x5118,Const(true))
    val x5121 = FringeContainer(x5099,b5621,b5622,x5111).name("x5121").ctrl(x5128) // FringeDenseLoad(x5099,x5110,x5111)
    val x5122 = Counter(min=Const(0), max=Const(9), step=Const(1), par=1).name("x5122").ctrl(x5128) // CounterNew(Const(0),Const(9),Const(1),Const(1))
    ...
    val b2556 = CounterIter(x5122, None).ctrl(x5127).name("b2556")
    val b2557 = DummyOp().ctrl(x5127).name("b2557")
    val x5124_x5124 = ReadMem(x5111).name("x5124").ctrl(x5127) // ParStreamRead(x5111,List(b2557))
    val x5125_x5125 = x5124_x5124 // x5125 = VectorApply(x5124,0)
    val x5126 = StoreBanks(List(x5108_d0_b0, x5108_d5_b0, x5108_d1_b0, x5108_d6_b0, x5108_d2_b0, x5108_d7_b0, x5108_d3_b0, x5108_d4_b0), List(b2556), x5125_x5125).name("x5126").ctrl(x5127) // ParSRAMStore(x5108,List(List(b2556)),List(x5125),List(b2557))
    val x5129 = Counter(min=Const(0), max=Const(32), step=Const(1), par=1).name("x5129").ctrl(x5568) // CounterNew(Const(0),Const(32),Const(1),Const(1))
    ...
    val b2565 = CounterIter(x5129, Some(0)).ctrl(x5153).name("b2565")
    val b2566 = DummyOp().ctrl(x5153).name("b2566")
    val b5625 = StreamOut(field="offset").name("b5625").ctrl(x5153) // x5131 = StreamOutNew(BurstCmdBus)
    val b5626 = StreamOut(field="size").name("b5626").ctrl(x5153) // x5131 = StreamOutNew(BurstCmdBus)
    val x5132 = StreamIn(field="data").name("x5132").ctrl(x5153) // x5132 = StreamInNew(BurstDataBus())
    val x5144 = UnitController(style=SeqPipe, level=InnerControl).name("x5144").ctrl(x5153) // UnitPipe(List(b2566),Block(x5143))
    val x5133 = b2565 // FixConvert(b2565,TRUE,_32,_0)
    val x5134 = OpDef(op=FixSla, inputs=List(x5133, Const(6))).name("x5134").ctrl(x5144) // FixLsh(x5133,Const(6))
    val x5135 = Const(0) // FixConvert(Const(0),TRUE,_32,_0)
    val x5136 = OpDef(op=FixAdd, inputs=List(x5134, x5135)).name("x5136").ctrl(x5144) // FixAdd(x5134,x5135)
    val x5137 = OpDef(op=FixSla, inputs=List(x5136, Const(2))).name("x5137").ctrl(x5144) // FixLsh(x5136,Const(2))
    val x5138 = x5137 // FixConvert(x5137,TRUE,_64,_0)
    val x5139 = top.argFringe.dramAddress(x5100).name("x5139").ctrl(x5144) // GetDRAMAddress(x5100)
    val x5140 = OpDef(op=FixAdd, inputs=List(x5138, x5139)).name("x5140").ctrl(x5144) // FixAdd(x5138,x5139)
    val x5142_x5141 = x5140 // FixConvert(x5140,TRUE,_64,_0)
    // x5142 = SimpleStruct(ArrayBuffer((offset,x5141), (size,Const(256)), (isLoad,Const(true))))
    val b5627_b5625 = WriteMem(b5625, x5142_x5141).name("b5627_b5625").ctrl(x5144) // StreamWrite(x5131,x5142,b2566)
    val b5628_b5626 = WriteMem(b5626, Const(256)).name("b5628_b5626").ctrl(x5144) // StreamWrite(x5131,x5142,b2566)
    val x5145 = FringeContainer(x5100,b5625,b5626,x5132).name("x5145").ctrl(x5153) // FringeDenseLoad(x5100,x5131,x5132)
    val x5146 = Counter(min=Const(0), max=Const(64), step=Const(1), par=16).name("x5146").ctrl(x5153) // CounterNew(Const(0),Const(64),Const(1),Const(16))
    ...
    isAccum(x5154_d0_b3) = false
    val x5155 = Counter(min=Const(0), max=Const(32), step=Const(1), par=1).name("x5155").ctrl(x5568) // CounterNew(Const(0),Const(32),Const(1),Const(1))
    ...
    val b2595 = CounterIter(x5155, Some(0)).ctrl(x5179).name("b2595")
    val b2596 = DummyOp().ctrl(x5179).name("b2596")
    val b5629 = StreamOut(field="offset").name("b5629").ctrl(x5179) // x5157 = StreamOutNew(BurstCmdBus)
    val b5630 = StreamOut(field="size").name("b5630").ctrl(x5179) // x5157 = StreamOutNew(BurstCmdBus)
    val x5158 = StreamIn(field="data").name("x5158").ctrl(x5179) // x5158 = StreamInNew(BurstDataBus())
    val x5170 = UnitController(style=SeqPipe, level=InnerControl).name("x5170").ctrl(x5179) // UnitPipe(List(b2596),Block(x5169))
    val x5159 = b2595 // FixConvert(b2595,TRUE,_32,_0)
    val x5160 = OpDef(op=FixSla, inputs=List(x5159, Const(6))).name("x5160").ctrl(x5170) // FixLsh(x5159,Const(6))
    val x5161 = Const(0) // FixConvert(Const(0),TRUE,_32,_0)
    val x5162 = OpDef(op=FixAdd, inputs=List(x5160, x5161)).name("x5162").ctrl(x5170) // FixAdd(x5160,x5161)
    val x5163 = OpDef(op=FixSla, inputs=List(x5162, Const(2))).name("x5163").ctrl(x5170) // FixLsh(x5162,Const(2))
    val x5164 = x5163 // FixConvert(x5163,TRUE,_64,_0)
    val x5165 = top.argFringe.dramAddress(x5101).name("x5165").ctrl(x5170) // GetDRAMAddress(x5101)
    val x5166 = OpDef(op=FixAdd, inputs=List(x5164, x5165)).name("x5166").ctrl(x5170) // FixAdd(x5164,x5165)
    val x5168_x5167 = x5166 // FixConvert(x5166,TRUE,_64,_0)
    // x5168 = SimpleStruct(ArrayBuffer((offset,x5167), (size,Const(256)), (isLoad,Const(true))))
    val b5631_b5629 = WriteMem(b5629, x5168_x5167).name("b5631_b5629").ctrl(x5170) // StreamWrite(x5157,x5168,b2596)
    val b5632_b5630 = WriteMem(b5630, Const(256)).name("b5632_b5630").ctrl(x5170) // StreamWrite(x5157,x5168,b2596)
    val x5171 = FringeContainer(x5101,b5629,b5630,x5158).name("x5171").ctrl(x5179) // FringeDenseLoad(x5101,x5157,x5158)
    val x5172 = Counter(min=Const(0), max=Const(64), step=Const(1), par=16).name("x5172").ctrl(x5179) // CounterNew(Const(0),Const(64),Const(1),Const(16))
    ...
    val b2614 = CounterIter(x5172, None).ctrl(x5178).name("b2614")
    val b2615 = DummyOp().ctrl(x5178).name("b2615")
    val x5174 = OpDef(op=BitAnd, inputs=List(b2615, b2596)).name("x5174").ctrl(x5178) // And(b2615,b2596)
    val x5175_x5175 = ReadMem(x5158).name("x5175").ctrl(x5178) // ParStreamRead(x5158,List(x5174))
    val x5176_x5176 = x5175_x5175 // x5176 = VectorApply(x5175,0)
    val x5177 = StoreBanks(List(x5154_d0_b0, x5154_d0_b1, x5154_d0_b2, x5154_d0_b3), List(b2595, b2614), x5176_x5176).name("x5177").ctrl(x5178) // ParSRAMStore(x5154,List(List(b2595, b2614)),List(x5176),List(x5174))
    val x5180 = ReadMem(x5090).name("x5180").ctrl(x5568) // RegRead(x5090)
    val x5181 = Counter(min=Const(0), max=x5180, step=Const(1), par=1).name("x5181").ctrl(x5568) // CounterNew(Const(0),x5180,Const(1),Const(1))
    ...
    val b2625 = CounterIter(x5181, Some(0)).ctrl(x5538).name("b2625")
    val b2626 = DummyOp().ctrl(x5538).name("b2626")
    val x5183 = Counter(min=Const(0), max=Const(32), step=Const(1), par=4).name("x5183").ctrl(x5538) // CounterNew(Const(0),Const(32),Const(1),Const(4))
    ...
    val b2629 = CounterIter(x5183, Some(0)).ctrl(x5537).name("b2629")
    val b2633 = DummyOp().ctrl(x5537).name("b2633")
    val b2630 = CounterIter(x5183, Some(1)).ctrl(x5537).name("b2630")
    val b2634 = DummyOp().ctrl(x5537).name("b2634")
    val b2631 = CounterIter(x5183, Some(2)).ctrl(x5537).name("b2631")
    val b2635 = DummyOp().ctrl(x5537).name("b2635")
    val b2632 = CounterIter(x5183, Some(3)).ctrl(x5537).name("b2632")
    val b2636 = DummyOp().ctrl(x5537).name("b2636")
    ...
    val x5194_x5185 = WriteMem(x5185, x5192).name("x5194_x5185").ctrl(x5195) // RegWrite(x5185,x5192,x5193)
    ...
    val x5201_x5186 = WriteMem(x5186, x5199).name("x5201_x5186").ctrl(x5202) // RegWrite(x5186,x5199,x5200)
    ...
    val x5208_x5187 = WriteMem(x5187, x5206).name("x5208_x5187").ctrl(x5209) // RegWrite(x5187,x5206,x5207)
    ...
    val x5215_x5188 = WriteMem(x5188, x5213).name("x5215_x5188").ctrl(x5216) // RegWrite(x5188,x5213,x5214)
    val x5217 = ReadMem(x5185).name("x5217").ctrl(x5537) // RegRead(x5185)
    val x5218 = Counter(min=x5217, max=Const(64), step=Const(1), par=1).name("x5218").ctrl(x5537) // CounterNew(x5217,Const(64),Const(1),Const(1))
    ...
    val b2674 = CounterIter(x5218, Some(0)).ctrl(x5296).name("b2674")
    val b2675 = DummyOp().ctrl(x5296).name("b2675")
    ...
    val x5224 = OpDef(op=FixAdd, inputs=List(b2629, Const(1))).name("x5224").ctrl(x5280) // FixAdd(b2629,Const(1))
    ...
    val x5230 = LoadBanks(List(x5109_d8_b0, x5109_d8_b1, x5109_d8_b2, x5109_d8_b3), List(x5227, b2674)).name("x5230").ctrl(x5280) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5227, b2674),Const(0),x5229)
    val x5231 = OpDef(op=FixAdd, inputs=List(b2674, Const(1))).name("x5231").ctrl(x5280) // FixAdd(b2674,Const(1))
    ...
    val x5235 = LoadBanks(List(x5109_d7_b0), List(b2629, x5234)).name("x5235").ctrl(x5280) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2629, x5234),Const(0),x5229)
    val x5236 = OpDef(op=FixSub, inputs=List(b2629, Const(1))).name("x5236").ctrl(x5280) // FixSub(b2629,Const(1))
    ...
    val x5240 = LoadBanks(List(x5109_d3_b0, x5109_d3_b1, x5109_d3_b2, x5109_d3_b3), List(x5239, b2674)).name("x5240").ctrl(x5280) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5239, b2674),Const(0),x5229)
    val x5241 = OpDef(op=FixSub, inputs=List(b2674, Const(1))).name("x5241").ctrl(x5280) // FixSub(b2674,Const(1))
    ...
    val x5245 = LoadBanks(List(x5109_d2_b0), List(b2629, x5244)).name("x5245").ctrl(x5280) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2629, x5244),Const(0),x5229)
    val x5246 = LoadBanks(List(x5109_d1_b0), List(b2629, b2674)).name("x5246").ctrl(x5280) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2629, b2674),Const(0),x5229)
    ...
    val x5252 = OpDef(op=FixAdd, inputs=List(x5251, Const(4))).name("x5252").ctrl(x5280) // FixAdd(x5251,Const(4))
    val x5253 = LoadBanks(List(x5108_d4_b0), List(x5252)).name("x5253").ctrl(x5280) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5252),Const(0),x5229)
    val x5254 = OpDef(op=FixAdd, inputs=List(x5250, Const(4))).name("x5254").ctrl(x5280) // FixAdd(x5250,Const(4))
    val x5255 = LoadBanks(List(x5108_d0_b0), List(x5254)).name("x5255").ctrl(x5280) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5254),Const(0),x5229)
    val x5256 = LoadBanks(List(x5154_d0_b0), List(b2629, b2674)).name("x5256").ctrl(x5280) // SRAMLoad(x5154,ArrayBuffer(Const(32), Const(64)),List(b2629, b2674),Const(0),x5229)
    val x5257 = OpDef(op=FixMul, inputs=List(x5256, x5246)).name("x5257").ctrl(x5280) // FixMul(x5256,x5246)
    val x5258 = OpDef(op=FixLt, inputs=List(x5257, Const(0))).name("x5258").ctrl(x5280) // FixLt(x5257,Const(0))
    val x5259 = ReadMem(x5092_d0).name("x5259").ctrl(x5280) // RegRead(x5092)
    val x5260 = ReadMem(x5091_d0).name("x5260").ctrl(x5280) // RegRead(x5091)
    ...
    val x5263 = OpDef(op=FixMin, inputs=List(Const(1.0), x5262)).name("x5263").ctrl(x5280) // Min(Const(1),x5262)
    val x5264 = OpDef(op=FixUnif, inputs=List()).name("x5264").ctrl(x5280) // FixUnif()
    val x5265 = OpDef(op=FixLt, inputs=List(Const(1.0), x5262)).name("x5265").ctrl(x5280) // FixLt(Const(1),x5262)
    ...
    val x5270 = OpDef(op=MuxOp, inputs=List(x5269, Const(1.0), Const(0.0))).name("x5270").ctrl(x5280) // Mux(x5269,Const(1),Const(0))
    val x5271 = OpDef(op=MuxOp, inputs=List(x5265, Const(1.0), x5270)).name("x5271").ctrl(x5280) // Mux(x5265,Const(1),x5270)
    val x5272 = OpDef(op=FixLeq, inputs=List(Const(0), b2674)).name("x5272").ctrl(x5280) // FixLeq(Const(0),b2674)
    val x5273 = OpDef(op=FixLt, inputs=List(b2674, Const(64))).name("x5273").ctrl(x5280) // FixLt(b2674,Const(64))
    ...
    val x5276_x5220 = WriteMem(x5220, x5246).name("x5276_x5220").ctrl(x5280) // RegWrite(x5220,x5246,x5229)
    val x5277_x5221 = WriteMem(x5221, x5271).name("x5277_x5221").ctrl(x5280) // RegWrite(x5221,x5271,x5229)
    val x5278_x5222_d0 = WriteMem(x5222_d0, x5274).name("x5278_x5222_d0").ctrl(x5280) // RegWrite(x5222,x5274,x5229)
    val x5278_x5222_d1 = WriteMem(x5222_d1, x5274).name("x5278_x5222_d1").ctrl(x5280) // RegWrite(x5222,x5274,x5229)
    val x5279_x5223 = WriteMem(x5223, x5275).name("x5279_x5223").ctrl(x5280) // RegWrite(x5223,x5275,x5229)
    val x5281 = ReadMem(x5223).name("x5281").ctrl(x5296) // RegRead(x5223)
    val x5282 = ReadMem(x5222_d1).name("x5282").ctrl(x5296) // RegRead(x5222)
    ...
    val x5283 = ReadMem(x5221).name("x5283").ctrl(x5293) // RegRead(x5221)
    val x5284 = OpDef(op=FixEql, inputs=List(x5283, Const(1.0))).name("x5284").ctrl(x5293) // FixEql(x5283,Const(1))
    val x5285 = ReadMem(x5220).name("x5285").ctrl(x5293) // RegRead(x5220)
    ...
    val x5288 = ReadMem(x5222_d0).name("x5288").ctrl(x5293) // RegRead(x5222)
    ...
    val x5292 = StoreBanks(List(x5109_d0_b0, x5109_d5_b0, x5109_d10_b0, x5109_d1_b0, x5109_d6_b0, x5109_d9_b0, x5109_d2_b0, x5109_d7_b0, x5109_d3_b0, x5109_d11_b0, x5109_d8_b0, x5109_d4_b0), List(b2629, b2674), x5287).name("x5292").ctrl(x5293) // SRAMStore(x5109,ArrayBuffer(Const(32), Const(64)),List(b2629, b2674),Const(0),x5287,x5291)
    val x5294 = UnitController(style=SeqPipe, level=InnerControl).name("x5294").ctrl(x5295) // //TODO SwitchCase(Block(Const(())))
    val x5297 = ReadMem(x5186).name("x5297").ctrl(x5537) // RegRead(x5186)
    val x5298 = Counter(min=x5297, max=Const(64), step=Const(1), par=1).name("x5298").ctrl(x5537) // CounterNew(x5297,Const(64),Const(1),Const(1))
    ...
    val b2745 = CounterIter(x5298, Some(0)).ctrl(x5376).name("b2745")
    val b2746 = DummyOp().ctrl(x5376).name("b2746")
    ...
    val x5304 = OpDef(op=FixAdd, inputs=List(b2630, Const(1))).name("x5304").ctrl(x5360) // FixAdd(b2630,Const(1))
    ...
    val x5310 = LoadBanks(List(x5109_d9_b0, x5109_d9_b1, x5109_d9_b2, x5109_d9_b3), List(x5307, b2745)).name("x5310").ctrl(x5360) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5307, b2745),Const(0),x5309)
    val x5311 = OpDef(op=FixAdd, inputs=List(b2745, Const(1))).name("x5311").ctrl(x5360) // FixAdd(b2745,Const(1))
    ...
    val x5315 = LoadBanks(List(x5109_d7_b1), List(b2630, x5314)).name("x5315").ctrl(x5360) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2630, x5314),Const(0),x5309)
    val x5316 = OpDef(op=FixSub, inputs=List(b2630, Const(1))).name("x5316").ctrl(x5360) // FixSub(b2630,Const(1))
    ...
    val x5320 = LoadBanks(List(x5109_d4_b0, x5109_d4_b1, x5109_d4_b2, x5109_d4_b3), List(x5319, b2745)).name("x5320").ctrl(x5360) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5319, b2745),Const(0),x5309)
    val x5321 = OpDef(op=FixSub, inputs=List(b2745, Const(1))).name("x5321").ctrl(x5360) // FixSub(b2745,Const(1))
    ...
    val x5325 = LoadBanks(List(x5109_d2_b1), List(b2630, x5324)).name("x5325").ctrl(x5360) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2630, x5324),Const(0),x5309)
    val x5326 = LoadBanks(List(x5109_d1_b1), List(b2630, b2745)).name("x5326").ctrl(x5360) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2630, b2745),Const(0),x5309)
    ...
    val x5332 = OpDef(op=FixAdd, inputs=List(x5331, Const(4))).name("x5332").ctrl(x5360) // FixAdd(x5331,Const(4))
    val x5333 = LoadBanks(List(x5108_d5_b0), List(x5332)).name("x5333").ctrl(x5360) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5332),Const(0),x5309)
    val x5334 = OpDef(op=FixAdd, inputs=List(x5330, Const(4))).name("x5334").ctrl(x5360) // FixAdd(x5330,Const(4))
    val x5335 = LoadBanks(List(x5108_d1_b0), List(x5334)).name("x5335").ctrl(x5360) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5334),Const(0),x5309)
    val x5336 = LoadBanks(List(x5154_d0_b1), List(b2630, b2745)).name("x5336").ctrl(x5360) // SRAMLoad(x5154,ArrayBuffer(Const(32), Const(64)),List(b2630, b2745),Const(0),x5309)
    val x5337 = OpDef(op=FixMul, inputs=List(x5336, x5326)).name("x5337").ctrl(x5360) // FixMul(x5336,x5326)
    val x5338 = OpDef(op=FixLt, inputs=List(x5337, Const(0))).name("x5338").ctrl(x5360) // FixLt(x5337,Const(0))
    val x5339 = ReadMem(x5092_d0).name("x5339").ctrl(x5360) // RegRead(x5092)
    val x5340 = ReadMem(x5091_d0).name("x5340").ctrl(x5360) // RegRead(x5091)
    ...
    val x5343 = OpDef(op=FixMin, inputs=List(Const(1.0), x5342)).name("x5343").ctrl(x5360) // Min(Const(1),x5342)
    val x5344 = OpDef(op=FixUnif, inputs=List()).name("x5344").ctrl(x5360) // FixUnif()
    val x5345 = OpDef(op=FixLt, inputs=List(Const(1.0), x5342)).name("x5345").ctrl(x5360) // FixLt(Const(1),x5342)
    ...
    val x5350 = OpDef(op=MuxOp, inputs=List(x5349, Const(1.0), Const(0.0))).name("x5350").ctrl(x5360) // Mux(x5349,Const(1),Const(0))
    val x5351 = OpDef(op=MuxOp, inputs=List(x5345, Const(1.0), x5350)).name("x5351").ctrl(x5360) // Mux(x5345,Const(1),x5350)
    val x5352 = OpDef(op=FixLeq, inputs=List(Const(0), b2745)).name("x5352").ctrl(x5360) // FixLeq(Const(0),b2745)
    val x5353 = OpDef(op=FixLt, inputs=List(b2745, Const(64))).name("x5353").ctrl(x5360) // FixLt(b2745,Const(64))
    ...
    val x5356_x5300 = WriteMem(x5300, x5326).name("x5356_x5300").ctrl(x5360) // RegWrite(x5300,x5326,x5309)
    val x5357_x5301 = WriteMem(x5301, x5351).name("x5357_x5301").ctrl(x5360) // RegWrite(x5301,x5351,x5309)
    val x5358_x5302_d0 = WriteMem(x5302_d0, x5354).name("x5358_x5302_d0").ctrl(x5360) // RegWrite(x5302,x5354,x5309)
    val x5358_x5302_d1 = WriteMem(x5302_d1, x5354).name("x5358_x5302_d1").ctrl(x5360) // RegWrite(x5302,x5354,x5309)
    val x5359_x5303 = WriteMem(x5303, x5355).name("x5359_x5303").ctrl(x5360) // RegWrite(x5303,x5355,x5309)
    val x5361 = ReadMem(x5303).name("x5361").ctrl(x5376) // RegRead(x5303)
    val x5362 = ReadMem(x5302_d1).name("x5362").ctrl(x5376) // RegRead(x5302)
    ...
    val x5363 = ReadMem(x5301).name("x5363").ctrl(x5373) // RegRead(x5301)
    val x5364 = OpDef(op=FixEql, inputs=List(x5363, Const(1.0))).name("x5364").ctrl(x5373) // FixEql(x5363,Const(1))
    val x5365 = ReadMem(x5300).name("x5365").ctrl(x5373) // RegRead(x5300)
    ...
    val x5368 = ReadMem(x5302_d0).name("x5368").ctrl(x5373) // RegRead(x5302)
    ...
    val x5372 = StoreBanks(List(x5109_d0_b1, x5109_d5_b1, x5109_d10_b1, x5109_d1_b1, x5109_d6_b1, x5109_d9_b1, x5109_d2_b1, x5109_d7_b1, x5109_d3_b1, x5109_d11_b1, x5109_d8_b1, x5109_d4_b1), List(b2630, b2745), x5367).name("x5372").ctrl(x5373) // SRAMStore(x5109,ArrayBuffer(Const(32), Const(64)),List(b2630, b2745),Const(0),x5367,x5371)
    val x5374 = UnitController(style=SeqPipe, level=InnerControl).name("x5374").ctrl(x5375) // //TODO SwitchCase(Block(Const(())))
    val x5377 = ReadMem(x5187).name("x5377").ctrl(x5537) // RegRead(x5187)
    val x5378 = Counter(min=x5377, max=Const(64), step=Const(1), par=1).name("x5378").ctrl(x5537) // CounterNew(x5377,Const(64),Const(1),Const(1))
    ...
    val b2816 = CounterIter(x5378, Some(0)).ctrl(x5456).name("b2816")
    val b2817 = DummyOp().ctrl(x5456).name("b2817")
    ...
    val x5384 = OpDef(op=FixAdd, inputs=List(b2631, Const(1))).name("x5384").ctrl(x5440) // FixAdd(b2631,Const(1))
    ...
    val x5390 = LoadBanks(List(x5109_d10_b0, x5109_d10_b1, x5109_d10_b2, x5109_d10_b3), List(x5387, b2816)).name("x5390").ctrl(x5440) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5387, b2816),Const(0),x5389)
    val x5391 = OpDef(op=FixAdd, inputs=List(b2816, Const(1))).name("x5391").ctrl(x5440) // FixAdd(b2816,Const(1))
    ...
    val x5395 = LoadBanks(List(x5109_d7_b2), List(b2631, x5394)).name("x5395").ctrl(x5440) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2631, x5394),Const(0),x5389)
    val x5396 = OpDef(op=FixSub, inputs=List(b2631, Const(1))).name("x5396").ctrl(x5440) // FixSub(b2631,Const(1))
    ...
    val x5400 = LoadBanks(List(x5109_d5_b0, x5109_d5_b1, x5109_d5_b2, x5109_d5_b3), List(x5399, b2816)).name("x5400").ctrl(x5440) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5399, b2816),Const(0),x5389)
    val x5401 = OpDef(op=FixSub, inputs=List(b2816, Const(1))).name("x5401").ctrl(x5440) // FixSub(b2816,Const(1))
    ...
    val x5405 = LoadBanks(List(x5109_d2_b2), List(b2631, x5404)).name("x5405").ctrl(x5440) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2631, x5404),Const(0),x5389)
    val x5406 = LoadBanks(List(x5109_d1_b2), List(b2631, b2816)).name("x5406").ctrl(x5440) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2631, b2816),Const(0),x5389)
    ...
    val x5412 = OpDef(op=FixAdd, inputs=List(x5411, Const(4))).name("x5412").ctrl(x5440) // FixAdd(x5411,Const(4))
    val x5413 = LoadBanks(List(x5108_d6_b0), List(x5412)).name("x5413").ctrl(x5440) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5412),Const(0),x5389)
    val x5414 = OpDef(op=FixAdd, inputs=List(x5410, Const(4))).name("x5414").ctrl(x5440) // FixAdd(x5410,Const(4))
    val x5415 = LoadBanks(List(x5108_d2_b0), List(x5414)).name("x5415").ctrl(x5440) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5414),Const(0),x5389)
    val x5416 = LoadBanks(List(x5154_d0_b2), List(b2631, b2816)).name("x5416").ctrl(x5440) // SRAMLoad(x5154,ArrayBuffer(Const(32), Const(64)),List(b2631, b2816),Const(0),x5389)
    val x5417 = OpDef(op=FixMul, inputs=List(x5416, x5406)).name("x5417").ctrl(x5440) // FixMul(x5416,x5406)
    val x5418 = OpDef(op=FixLt, inputs=List(x5417, Const(0))).name("x5418").ctrl(x5440) // FixLt(x5417,Const(0))
    val x5419 = ReadMem(x5092_d0).name("x5419").ctrl(x5440) // RegRead(x5092)
    val x5420 = ReadMem(x5091_d0).name("x5420").ctrl(x5440) // RegRead(x5091)
    ...
    val x5423 = OpDef(op=FixMin, inputs=List(Const(1.0), x5422)).name("x5423").ctrl(x5440) // Min(Const(1),x5422)
    val x5424 = OpDef(op=FixUnif, inputs=List()).name("x5424").ctrl(x5440) // FixUnif()
    val x5425 = OpDef(op=FixLt, inputs=List(Const(1.0), x5422)).name("x5425").ctrl(x5440) // FixLt(Const(1),x5422)
    ...
    val x5430 = OpDef(op=MuxOp, inputs=List(x5429, Const(1.0), Const(0.0))).name("x5430").ctrl(x5440) // Mux(x5429,Const(1),Const(0))
    val x5431 = OpDef(op=MuxOp, inputs=List(x5425, Const(1.0), x5430)).name("x5431").ctrl(x5440) // Mux(x5425,Const(1),x5430)
    val x5432 = OpDef(op=FixLeq, inputs=List(Const(0), b2816)).name("x5432").ctrl(x5440) // FixLeq(Const(0),b2816)
    val x5433 = OpDef(op=FixLt, inputs=List(b2816, Const(64))).name("x5433").ctrl(x5440) // FixLt(b2816,Const(64))
    ...
    val x5436_x5380 = WriteMem(x5380, x5406).name("x5436_x5380").ctrl(x5440) // RegWrite(x5380,x5406,x5389)
    val x5437_x5381 = WriteMem(x5381, x5431).name("x5437_x5381").ctrl(x5440) // RegWrite(x5381,x5431,x5389)
    val x5438_x5382_d0 = WriteMem(x5382_d0, x5434).name("x5438_x5382_d0").ctrl(x5440) // RegWrite(x5382,x5434,x5389)
    val x5438_x5382_d1 = WriteMem(x5382_d1, x5434).name("x5438_x5382_d1").ctrl(x5440) // RegWrite(x5382,x5434,x5389)
    val x5439_x5383 = WriteMem(x5383, x5435).name("x5439_x5383").ctrl(x5440) // RegWrite(x5383,x5435,x5389)
    val x5441 = ReadMem(x5383).name("x5441").ctrl(x5456) // RegRead(x5383)
    val x5442 = ReadMem(x5382_d1).name("x5442").ctrl(x5456) // RegRead(x5382)
    ...
    val x5443 = ReadMem(x5381).name("x5443").ctrl(x5453) // RegRead(x5381)
    val x5444 = OpDef(op=FixEql, inputs=List(x5443, Const(1.0))).name("x5444").ctrl(x5453) // FixEql(x5443,Const(1))
    val x5445 = ReadMem(x5380).name("x5445").ctrl(x5453) // RegRead(x5380)
    ...
    val x5448 = ReadMem(x5382_d0).name("x5448").ctrl(x5453) // RegRead(x5382)
    ...
    val x5452 = StoreBanks(List(x5109_d0_b2, x5109_d5_b2, x5109_d10_b2, x5109_d1_b2, x5109_d6_b2, x5109_d9_b2, x5109_d2_b2, x5109_d7_b2, x5109_d3_b2, x5109_d11_b2, x5109_d8_b2, x5109_d4_b2), List(b2631, b2816), x5447).name("x5452").ctrl(x5453) // SRAMStore(x5109,ArrayBuffer(Const(32), Const(64)),List(b2631, b2816),Const(0),x5447,x5451)
    val x5454 = UnitController(style=SeqPipe, level=InnerControl).name("x5454").ctrl(x5455) // //TODO SwitchCase(Block(Const(())))
    val x5457 = ReadMem(x5188).name("x5457").ctrl(x5537) // RegRead(x5188)
    val x5458 = Counter(min=x5457, max=Const(64), step=Const(1), par=1).name("x5458").ctrl(x5537) // CounterNew(x5457,Const(64),Const(1),Const(1))
    ...
    val b2887 = CounterIter(x5458, Some(0)).ctrl(x5536).name("b2887")
    val b2888 = DummyOp().ctrl(x5536).name("b2888")
    ...
    val x5464 = OpDef(op=FixAdd, inputs=List(b2632, Const(1))).name("x5464").ctrl(x5520) // FixAdd(b2632,Const(1))
    ...
    val x5470 = LoadBanks(List(x5109_d11_b0, x5109_d11_b1, x5109_d11_b2, x5109_d11_b3), List(x5467, b2887)).name("x5470").ctrl(x5520) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5467, b2887),Const(0),x5469)
    val x5471 = OpDef(op=FixAdd, inputs=List(b2887, Const(1))).name("x5471").ctrl(x5520) // FixAdd(b2887,Const(1))
    ...
    val x5475 = LoadBanks(List(x5109_d7_b3), List(b2632, x5474)).name("x5475").ctrl(x5520) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2632, x5474),Const(0),x5469)
    val x5476 = OpDef(op=FixSub, inputs=List(b2632, Const(1))).name("x5476").ctrl(x5520) // FixSub(b2632,Const(1))
    ...
    val x5480 = LoadBanks(List(x5109_d6_b0, x5109_d6_b1, x5109_d6_b2, x5109_d6_b3), List(x5479, b2887)).name("x5480").ctrl(x5520) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(x5479, b2887),Const(0),x5469)
    val x5481 = OpDef(op=FixSub, inputs=List(b2887, Const(1))).name("x5481").ctrl(x5520) // FixSub(b2887,Const(1))
    ...
    val x5485 = LoadBanks(List(x5109_d2_b3), List(b2632, x5484)).name("x5485").ctrl(x5520) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2632, x5484),Const(0),x5469)
    val x5486 = LoadBanks(List(x5109_d1_b3), List(b2632, b2887)).name("x5486").ctrl(x5520) // SRAMLoad(x5109,ArrayBuffer(Const(32), Const(64)),List(b2632, b2887),Const(0),x5469)
    ...
    val x5492 = OpDef(op=FixAdd, inputs=List(x5491, Const(4))).name("x5492").ctrl(x5520) // FixAdd(x5491,Const(4))
    val x5493 = LoadBanks(List(x5108_d7_b0), List(x5492)).name("x5493").ctrl(x5520) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5492),Const(0),x5469)
    val x5494 = OpDef(op=FixAdd, inputs=List(x5490, Const(4))).name("x5494").ctrl(x5520) // FixAdd(x5490,Const(4))
    val x5495 = LoadBanks(List(x5108_d3_b0), List(x5494)).name("x5495").ctrl(x5520) // SRAMLoad(x5108,ArrayBuffer(Const(9)),List(x5494),Const(0),x5469)
    val x5496 = LoadBanks(List(x5154_d0_b3), List(b2632, b2887)).name("x5496").ctrl(x5520) // SRAMLoad(x5154,ArrayBuffer(Const(32), Const(64)),List(b2632, b2887),Const(0),x5469)
    val x5497 = OpDef(op=FixMul, inputs=List(x5496, x5486)).name("x5497").ctrl(x5520) // FixMul(x5496,x5486)
    val x5498 = OpDef(op=FixLt, inputs=List(x5497, Const(0))).name("x5498").ctrl(x5520) // FixLt(x5497,Const(0))
    val x5499 = ReadMem(x5092_d0).name("x5499").ctrl(x5520) // RegRead(x5092)
    val x5500 = ReadMem(x5091_d0).name("x5500").ctrl(x5520) // RegRead(x5091)
    ...
    val x5503 = OpDef(op=FixMin, inputs=List(Const(1.0), x5502)).name("x5503").ctrl(x5520) // Min(Const(1),x5502)
    val x5504 = OpDef(op=FixUnif, inputs=List()).name("x5504").ctrl(x5520) // FixUnif()
    val x5505 = OpDef(op=FixLt, inputs=List(Const(1.0), x5502)).name("x5505").ctrl(x5520) // FixLt(Const(1),x5502)
    ...
    val x5510 = OpDef(op=MuxOp, inputs=List(x5509, Const(1.0), Const(0.0))).name("x5510").ctrl(x5520) // Mux(x5509,Const(1),Const(0))
    val x5511 = OpDef(op=MuxOp, inputs=List(x5505, Const(1.0), x5510)).name("x5511").ctrl(x5520) // Mux(x5505,Const(1),x5510)
    val x5512 = OpDef(op=FixLeq, inputs=List(Const(0), b2887)).name("x5512").ctrl(x5520) // FixLeq(Const(0),b2887)
    val x5513 = OpDef(op=FixLt, inputs=List(b2887, Const(64))).name("x5513").ctrl(x5520) // FixLt(b2887,Const(64))
    ...
    val x5516_x5460 = WriteMem(x5460, x5486).name("x5516_x5460").ctrl(x5520) // RegWrite(x5460,x5486,x5469)
    val x5517_x5461 = WriteMem(x5461, x5511).name("x5517_x5461").ctrl(x5520) // RegWrite(x5461,x5511,x5469)
    val x5518_x5462_d0 = WriteMem(x5462_d0, x5514).name("x5518_x5462_d0").ctrl(x5520) // RegWrite(x5462,x5514,x5469)
    val x5518_x5462_d1 = WriteMem(x5462_d1, x5514).name("x5518_x5462_d1").ctrl(x5520) // RegWrite(x5462,x5514,x5469)
    val x5519_x5463 = WriteMem(x5463, x5515).name("x5519_x5463").ctrl(x5520) // RegWrite(x5463,x5515,x5469)
    val x5521 = ReadMem(x5463).name("x5521").ctrl(x5536) // RegRead(x5463)
    val x5522 = ReadMem(x5462_d1).name("x5522").ctrl(x5536) // RegRead(x5462)
    ...
    val x5523 = ReadMem(x5461).name("x5523").ctrl(x5533) // RegRead(x5461)
    val x5524 = OpDef(op=FixEql, inputs=List(x5523, Const(1.0))).name("x5524").ctrl(x5533) // FixEql(x5523,Const(1))
    val x5525 = ReadMem(x5460).name("x5525").ctrl(x5533) // RegRead(x5460)
    ...
    val x5528 = ReadMem(x5462_d0).name("x5528").ctrl(x5533) // RegRead(x5462)
    ...
    val x5532 = StoreBanks(List(x5109_d0_b3, x5109_d5_b3, x5109_d10_b3, x5109_d1_b3, x5109_d6_b3, x5109_d9_b3, x5109_d2_b3, x5109_d7_b3, x5109_d3_b3, x5109_d11_b3, x5109_d8_b3, x5109_d4_b3), List(b2632, b2887), x5527).name("x5532").ctrl(x5533) // SRAMStore(x5109,ArrayBuffer(Const(32), Const(64)),List(b2632, b2887),Const(0),x5527,x5531)
    val x5534 = UnitController(style=SeqPipe, level=InnerControl).name("x5534").ctrl(x5535) // //TODO SwitchCase(Block(Const(())))
    val x5539 = Counter(min=Const(0), max=Const(32), step=Const(1), par=1).name("x5539").ctrl(x5568) // CounterNew(Const(0),Const(32),Const(1),Const(1))
    ...
    val b2963 = CounterIter(x5539, Some(0)).ctrl(x5567).name("b2963")
    val b2964 = DummyOp().ctrl(x5567).name("b2964")
    val b5633 = StreamOut(field="offset").name("b5633").ctrl(x5567) // x5541 = StreamOutNew(BurstCmdBus)
    val b5634 = StreamOut(field="size").name("b5634").ctrl(x5567) // x5541 = StreamOutNew(BurstCmdBus)
    val x5542 = StreamOut(field="data").name("x5542").ctrl(x5567) // x5542 = StreamOutNew(BurstFullDataBus())
    val x5543 = StreamIn(field="ack").name("x5543").ctrl(x5567) // x5543 = StreamInNew(BurstAckBus)
    val x5555 = UnitController(style=SeqPipe, level=InnerControl).name("x5555").ctrl(x5567) // UnitPipe(List(b2964),Block(x5554))
    val x5544 = b2963 // FixConvert(b2963,TRUE,_32,_0)
    val x5545 = OpDef(op=FixSla, inputs=List(x5544, Const(6))).name("x5545").ctrl(x5555) // FixLsh(x5544,Const(6))
    val x5546 = Const(0) // FixConvert(Const(0),TRUE,_32,_0)
    val x5547 = OpDef(op=FixAdd, inputs=List(x5545, x5546)).name("x5547").ctrl(x5555) // FixAdd(x5545,x5546)
    val x5548 = OpDef(op=FixSla, inputs=List(x5547, Const(2))).name("x5548").ctrl(x5555) // FixLsh(x5547,Const(2))
    val x5549 = x5548 // FixConvert(x5548,TRUE,_64,_0)
    val x5550 = top.argFringe.dramAddress(x5100).name("x5550").ctrl(x5555) // GetDRAMAddress(x5100)
    val x5551 = OpDef(op=FixAdd, inputs=List(x5549, x5550)).name("x5551").ctrl(x5555) // FixAdd(x5549,x5550)
    val x5553_x5552 = x5551 // FixConvert(x5551,TRUE,_64,_0)
    // x5553 = SimpleStruct(ArrayBuffer((offset,x5552), (size,Const(256)), (isLoad,Const(false))))
    val b5635_b5633 = WriteMem(b5633, x5553_x5552).name("b5635_b5633").ctrl(x5555) // StreamWrite(x5541,x5553,b2964)
    val b5636_b5634 = WriteMem(b5634, Const(256)).name("b5636_b5634").ctrl(x5555) // StreamWrite(x5541,x5553,b2964)
    val x5556 = Counter(min=Const(0), max=Const(64), step=Const(1), par=16).name("x5556").ctrl(x5567) // CounterNew(Const(0),Const(64),Const(1),Const(16))
    ...
    val b2982 = CounterIter(x5556, None).ctrl(x5563).name("b2982")
    val b2983 = DummyOp().ctrl(x5563).name("b2983")
    val x5558 = OpDef(op=BitAnd, inputs=List(b2983, b2964)).name("x5558").ctrl(x5563) // And(b2983,b2964)
    val x5559 = LoadBanks(List(x5109_d0_b0, x5109_d0_b1, x5109_d0_b2, x5109_d0_b3), List(b2963, b2982)).name("x5559").ctrl(x5563) // ParSRAMLoad(x5109,List(List(b2963, b2982)),List(x5558))
    val x5561_x5560 = x5559 // x5560 = VectorApply(x5559,0)
    // x5561 = SimpleStruct(ArrayBuffer((_1,x5560), (_2,Const(true))))
    val x5562_x5542 = WriteMem(x5542, x5561_x5560).name("x5562_x5542").ctrl(x5563) // ParStreamWrite(x5542,List(x5561),List(x5558))
    val x5564 = FringeContainer(x5100,b5633,b5634,x5542,x5543).name("x5564").ctrl(x5567) // FringeDenseStore(x5100,x5541,x5542,x5543)
    val x5566 = UnitController(style=SeqPipe, level=InnerControl).name("x5566").ctrl(x5567) // UnitPipe(List(b2964),Block(Const(())))
    val x5565_x5565 = ReadMem(x5543).name("x5565").ctrl(x5566) // StreamRead(x5543,b2964)
