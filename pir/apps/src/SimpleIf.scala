...
object SimpleIf extends PIRApp {
  def main(top:Top) = {
    ...
    val b347 = IterDef(x734, Some(0)).ctrl(x796).name("b347")
    val b350 = DummyDef().ctrl(x796).name("b350")
    val b348 = IterDef(x734, Some(1)).ctrl(x796).name("b348")
    val b351 = DummyDef().ctrl(x796).name("b351")
    val b349 = IterDef(x734, Some(2)).ctrl(x796).name("b349")
    val b352 = DummyDef().ctrl(x796).name("b352")
    ...
    val x742 = LoadDef(List(x730_d0), None).name("x742").ctrl(x747) // RegRead(x730)
    ...
    val x745 = StoreDef(List(x736_d0, x736_d1), None, x743).name("x745").ctrl(x747) // RegWrite(x736,x743,b350)
    val x746 = StoreDef(List(x739_d0, x739_d1), None, x744).name("x746").ctrl(x747) // RegWrite(x739,x744,b350)
    val x753 = UnitController(style=SeqPipe, level=InnerControl).name("x753").ctrl(x796) // UnitPipe(List(b351),Block(Const(())))
    val x748 = LoadDef(List(x730_d0), None).name("x748").ctrl(x753) // RegRead(x730)
    ...
    val x751 = StoreDef(List(x737_d0, x737_d1), None, x749).name("x751").ctrl(x753) // RegWrite(x737,x749,b351)
    val x752 = StoreDef(List(x740_d0, x740_d1), None, x750).name("x752").ctrl(x753) // RegWrite(x740,x750,b351)
    val x759 = UnitController(style=SeqPipe, level=InnerControl).name("x759").ctrl(x796) // UnitPipe(List(b352),Block(Const(())))
    val x754 = LoadDef(List(x730_d0), None).name("x754").ctrl(x759) // RegRead(x730)
    ...
    val x757 = StoreDef(List(x738_d0, x738_d1), None, x755).name("x757").ctrl(x759) // RegWrite(x738,x755,b352)
    val x758 = StoreDef(List(x741_d0, x741_d1), None, x756).name("x758").ctrl(x759) // RegWrite(x741,x756,b352)
    val x760 = LoadDef(List(x739_d1), None).name("x760").ctrl(x796) // RegRead(x739)
    val x761 = LoadDef(List(x736_d1), None).name("x761").ctrl(x796) // RegRead(x736)
    ...
    val x762 = LoadDef(List(x736_d0), None).name("x762").ctrl(x765) // RegRead(x736)
    val x763 = OpDef(op=BitAnd, inputs=List(x762, b350)).name("x763").ctrl(x765) // And(x762,b350)
    val x764 = StoreDef(List(x733_d0_b0), Some(List(b347)), b347).name("x764").ctrl(x765) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b347),Const(0),b347,x763)
    ...
    val x767 = LoadDef(List(x739_d0), None).name("x767").ctrl(x770) // RegRead(x739)
    val x768 = OpDef(op=BitAnd, inputs=List(x767, b350)).name("x768").ctrl(x770) // And(x767,b350)
    val x769 = StoreDef(List(x733_d0_b0), Some(List(b347)), x766).name("x769").ctrl(x770) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b347),Const(0),x766,x768)
    val x772 = LoadDef(List(x740_d1), None).name("x772").ctrl(x796) // RegRead(x740)
    val x773 = LoadDef(List(x737_d1), None).name("x773").ctrl(x796) // RegRead(x737)
    ...
    val x774 = LoadDef(List(x737_d0), None).name("x774").ctrl(x777) // RegRead(x737)
    val x775 = OpDef(op=BitAnd, inputs=List(x774, b351)).name("x775").ctrl(x777) // And(x774,b351)
    val x776 = StoreDef(List(x733_d0_b1), Some(List(b348)), b348).name("x776").ctrl(x777) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b348),Const(0),b348,x775)
    ...
    val x779 = LoadDef(List(x740_d0), None).name("x779").ctrl(x782) // RegRead(x740)
    val x780 = OpDef(op=BitAnd, inputs=List(x779, b351)).name("x780").ctrl(x782) // And(x779,b351)
    val x781 = StoreDef(List(x733_d0_b1), Some(List(b348)), x778).name("x781").ctrl(x782) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b348),Const(0),x778,x780)
    val x784 = LoadDef(List(x741_d1), None).name("x784").ctrl(x796) // RegRead(x741)
    val x785 = LoadDef(List(x738_d1), None).name("x785").ctrl(x796) // RegRead(x738)
    ...
    val x786 = LoadDef(List(x738_d0), None).name("x786").ctrl(x789) // RegRead(x738)
    val x787 = OpDef(op=BitAnd, inputs=List(x786, b352)).name("x787").ctrl(x789) // And(x786,b352)
    val x788 = StoreDef(List(x733_d0_b2), Some(List(b349)), b349).name("x788").ctrl(x789) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b349),Const(0),b349,x787)
    ...
    val x791 = LoadDef(List(x741_d0), None).name("x791").ctrl(x794) // RegRead(x741)
    val x792 = OpDef(op=BitAnd, inputs=List(x791, b352)).name("x792").ctrl(x794) // And(x791,b352)
    val x793 = StoreDef(List(x733_d0_b2), Some(List(b349)), x790).name("x793").ctrl(x794) // SRAMStore(x733,ArrayBuffer(Const(3)),List(b349),Const(0),x790,x792)
    val x799 = UnitController(style=SeqPipe, level=InnerControl).name("x799").ctrl(x800) // UnitPipe(List(Const(true)),Block(Const(())))
    val x797 = LoadDef(List(x733_d0_b0, x733_d0_b1, x733_d0_b2), Some(List(Const(2)))).name("x797").ctrl(x799) // SRAMLoad(x733,ArrayBuffer(Const(3)),List(Const(2)),Const(0),Const(true))
    val x798 = StoreDef(List(x731), None, x797).name("x798").ctrl(x799) // RegWrite(x731,x797,Const(true))
