...
object DotProduct extends PIRApp {
  def main(top:Top) = {
    ...
    val a_oc = OffChip("a")
    val x1466_x1515_ra_v = Vector("x1466_x1515_ra")
    val a_da = DRAMAddress("a", "a")
    val x1490_x1500_data_v = Vector("x1490_x1500_data")
    val x1466_x1506_wa_v = Vector("x1466_x1506_wa")
    val b_da = DRAMAddress("b", "b")
    val x1465_x1486_wa_v = Vector("x1465_x1486_wa")
    ...
    val x1465_x1513_ra_v = Vector("x1465_x1513_ra")
    ...
    val x1465_dsp0_bank0 = MemoryPipeline(name="x1465_dsp0_bank0",parent="x1529") { implicit CU => 
      val b1562 = VectorFIFO(size=1,name="b1562").wtPort(x1465_x1513_ra_v)
      val b1556 = VectorFIFO(size=1,name="b1556").wtPort(x1465_x1486_wa_v)
      val x1486 = VectorFIFO(size=1,name="x1486").wtPort(x1470_x1480_data_v)
      val x1465 = SRAM(size=1,name="x1465",banking = Strided(1,16)).buffering(2).wtPort(x1486.readPort).rdPort(x1465_x1465_dsp0_bank0_data_v).rdAddr(b1562.readPort).wtAddr(b1556.readPort).consumer("x1513_0", "x1522_0").producer("x1487_0", "x1488")
    }
    val x1466_dsp0_bank0 = MemoryPipeline(name="x1466_dsp0_bank0",parent="x1529") { implicit CU => 
      val b1563 = VectorFIFO(size=1,name="b1563").wtPort(x1466_x1515_ra_v)
      val x1506 = VectorFIFO(size=1,name="x1506").wtPort(x1490_x1500_data_v)
      val b1561 = VectorFIFO(size=1,name="b1561").wtPort(x1466_x1506_wa_v)
      val x1466 = SRAM(size=1,name="x1466",banking = Strided(1,16)).buffering(2).wtPort(x1506.readPort).rdPort(x1466_x1466_dsp0_bank0_data_v).rdAddr(b1563.readPort).wtAddr(b1561.readPort).consumer("x1515_0", "x1522_0").producer("x1507_0", "x1508")
    }
