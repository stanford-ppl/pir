...
object DotProduct extends PIRApp {
  def main(implicit design:PIRDesign) = {
    ...
    val x1461_d0 = Reg(init=Some(0)).name("x1461_d0").ctrl(x1533) // x1461 = RegNew(Const(0))
    isAccum(x1461_d0) = false
    bufferDepthOf(x1461_d0) = 1
    val x1461_d1 = Reg(init=Some(0)).name("x1461_d1").ctrl(x1533) // x1461 = RegNew(Const(0))
    isAccum(x1461_d1) = true
    bufferDepthOf(x1461_d1) = 1
    val x1462 = ReadMem(x1451).name("x1462").ctrl(x1533) // RegRead(x1451)
    val x1463 = Counter(min=Const(0), max=x1462, step=Const(32), par=1).name("x1463").ctrl(x1533) // CounterNew(Const(0),x1462,Const(32),Const(1))
    ...
    val x1465_d0_b0 = SRAM(size=2, banking=Strided(banks=16, stride=1)).name("x1465_d0_b0").ctrl(x1529) // x1465 = SRAMNew(ArrayBuffer(Const(32)))
    isAccum(x1465_d0_b0) = false
    bufferDepthOf(x1465_d0_b0) = 2
    val x1466_d0_b0 = SRAM(size=2, banking=Strided(banks=16, stride=1)).name("x1466_d0_b0").ctrl(x1529) // x1466 = SRAMNew(ArrayBuffer(Const(32)))
    isAccum(x1466_d0_b0) = false
    bufferDepthOf(x1466_d0_b0) = 2
    val x1468 = UnitController(style=SeqPipe, level=InnerControl).name("x1468").ctrl(x1529) // UnitPipe(List(b929),Block(Const(())))
    val x1467 = OpDef(op=FixAdd, inputs=List(b928, Const(32))).name("x1467").ctrl(x1468) // FixAdd(b928,Const(32))
    ...
    val x1477_x1476 = x1475 // FixConvert(x1475,TRUE,_64,_0)
    // x1477 = SimpleStruct(ArrayBuffer((offset,x1476), (size,Const(128)), (isLoad,Const(true))))
    val b1554_b1552 = WriteMem(b1552, x1477_x1476).name("b1554_b1552").ctrl(x1479) // StreamWrite(x1469,x1477,b929)
    val b1555_b1553 = WriteMem(b1553, Const(128)).name("b1555_b1553").ctrl(x1479) // StreamWrite(x1469,x1477,b929)
    val x1480 = FringeContainer(x1455,b1552,b1553,x1470).name("x1480").ctrl(x1488) // FringeDenseLoad(x1455,x1469,x1470)
    val x1481 = Counter(min=Const(0), max=Const(32), step=Const(1), par=16).name("x1481").ctrl(x1488) // CounterNew(Const(0),Const(32),Const(1),Const(16))
    ...
    val x1484_x1484 = ReadMem(x1470).name("x1484").ctrl(x1487) // ParStreamRead(x1470,List(x1483))
    val x1485_x1485 = x1484_x1484 // x1485 = VectorApply(x1484,0)
    val x1486 = StoreBanks(List(x1465_d0_b0), List(b948), x1485_x1485).name("x1486").ctrl(x1487) // ParSRAMStore(x1465,List(List(b948)),List(x1485),List(x1483))
    ...
    val x1497_x1496 = x1495 // FixConvert(x1495,TRUE,_64,_0)
    // x1497 = SimpleStruct(ArrayBuffer((offset,x1496), (size,Const(128)), (isLoad,Const(true))))
    val b1558_b1556 = WriteMem(b1556, x1497_x1496).name("b1558_b1556").ctrl(x1499) // StreamWrite(x1489,x1497,b929)
    val b1559_b1557 = WriteMem(b1557, Const(128)).name("b1559_b1557").ctrl(x1499) // StreamWrite(x1489,x1497,b929)
    val x1500 = FringeContainer(x1457,b1556,b1557,x1490).name("x1500").ctrl(x1508) // FringeDenseLoad(x1457,x1489,x1490)
    val x1501 = Counter(min=Const(0), max=Const(32), step=Const(1), par=16).name("x1501").ctrl(x1508) // CounterNew(Const(0),Const(32),Const(1),Const(16))
    ...
    val x1504_x1504 = ReadMem(x1490).name("x1504").ctrl(x1507) // ParStreamRead(x1490,List(x1503))
    val x1505_x1505 = x1504_x1504 // x1505 = VectorApply(x1504,0)
    val x1506 = StoreBanks(List(x1466_d0_b0), List(b970), x1505_x1505).name("x1506").ctrl(x1507) // ParSRAMStore(x1466,List(List(b970)),List(x1505),List(x1503))
    val x1509_d0 = Reg(init=Some(0)).name("x1509_d0").ctrl(x1529) // x1509 = RegNew(Const(0))
    isAccum(x1509_d0) = false
    bufferDepthOf(x1509_d0) = 2
    val x1509_d1 = Reg(init=Some(0)).name("x1509_d1").ctrl(x1529) // x1509 = RegNew(Const(0))
    isAccum(x1509_d1) = true
    bufferDepthOf(x1509_d1) = 1
    val x1510 = Counter(min=Const(0), max=Const(32), step=Const(1), par=16).name("x1510").ctrl(x1529) // CounterNew(Const(0),Const(32),Const(1),Const(16))
