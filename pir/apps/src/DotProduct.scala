...
object DotProduct extends PIRApp {
  def main(top:Top) = {
    ...
    val x1465_dsp0_bank0 = MemoryPipeline(name="x1465_dsp0_bank0",parent="x1529") { implicit CU => 
      ...
      val x1465 = SRAM(size=1,name="x1465",banking = Strided(1,16)).wtPort(x1486.readPort).rdPort(x1465_x1465_dsp0_bank0_data_s).rdAddr(b1562.readPort).wtAddr(b1556.readPort).consumer("x1513_0", "x1522").producer("x1487_0", "x1488")
    }
    val x1466_dsp0_bank0 = MemoryPipeline(name="x1466_dsp0_bank0",parent="x1529") { implicit CU => 
      ...
      val x1466 = SRAM(size=1,name="x1466",banking = Strided(1,16)).wtPort(x1506.readPort).rdPort(x1466_x1466_dsp0_bank0_data_s).rdAddr(b1563.readPort).wtAddr(b1561.readPort).consumer("x1515_0", "x1522").producer("x1507_0", "x1508")
    }
    ...
    val x1522_1 = Pipeline(name="x1522_1",parent="x1522") { implicit CU => 
      val rr91 = VectorFIFO(size=1,name="rr91").wtPort(bus_91_v)
      Stage(operands=List(CU.load(rr91)), op=Bypass, results=List(CU.reduce))
    ...
    val x1522 = StreamController(name="x1522",parent="x1529") { implicit CU => 
      val ctr4 = Counter(min=Const(0), max=Const(16), step=Const(1), par=16) // Counter
      val x1511 = CounterChain(name = "x1511", ctr4).iter(1)
    }
    val x1522_0 = Pipeline(name="x1522_0",parent="x1522") { implicit CU => 
      ...
      Stage(operands=List(CU.load(x1513), CU.load(x1515)), op=FixMul, results=List(CU.vecOut(bus_91_v)))
    }
    ...
    val x1528_0 = Pipeline(name="x1528_0",parent="x1529") { implicit CU => 
      val x1509 = ScalarBuffer(name="x1509").wtPort(x1509_x1521_s).consumer("x1528_0", "x1528_0")
