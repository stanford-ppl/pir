...
object OuterProductDesign extends PIRApp {
  ...
  def main(args: String*)(top:Top) = {
    val x3775_vector = Vector()
    val x3683_argin = ArgIn()
    val x3778_vector = Vector()
    val x3825_scalar = Scalar()
    val x3781_scalar = Scalar()
    val x3689_oc = OffChip()
    val x3906_vector = Vector()
    val x3823_vector = Vector()
    val x3691_oc = OffChip()
    val x3777_scalar = Scalar()
    val x3782_scalar = Scalar()
    val x3827_scalar = Scalar()
    val x3909_scalar = Scalar()
    val x3780_scalar = Scalar()
    val x3826_scalar = Scalar()
    val x3779_scalar = Scalar()
    val x3687_oc = OffChip()
    val x3824_scalar = Scalar()
    val x3682_argin = ArgIn()
    val x3776_scalar = Scalar()
    ...
    val x3801 = TileTransfer(name="x3801", parent=x3822, memctrl=x3801_mc_mc, mctpe=TileLoad, deps=List(x3797), vec=x3778_vector) { implicit CU => 
      ...
      CounterChain(name = "x3801_cc_stream", x3801_ctr).isStreaming(true)
      var stage: List[Stage] = Nil
      stage = stage0 +: Stages(1)
      Stage(stage(1), operands=List(CU.scalarIn(stage(0), x3781_scalar), CU.ctr(stage(0), x3801_cc(0))), op=FixAdd, results=List(CU.scalarOut(stage(1), x3801_mc_mc.saddr)))
    }
    ...
    val x3846 = TileTransfer(name="x3846", parent=x3867, memctrl=x3846_mc_mc, mctpe=TileLoad, deps=List(x3842), vec=x3823_vector) { implicit CU => 
      ...
      CounterChain(name = "x3846_cc_stream", x3846_ctr).isStreaming(true)
      var stage: List[Stage] = Nil
      stage = stage0 +: Stages(1)
      Stage(stage(1), operands=List(CU.scalarIn(stage(0), x3826_scalar), CU.ctr(stage(0), x3846_cc(0))), op=FixAdd, results=List(CU.scalarOut(stage(1), x3846_mc_mc.saddr)))
    }
    ...
    val x3927 = TileTransfer(name="x3927", parent=x3929, memctrl=x3927_mc_mc, mctpe=TileStore, deps=List(x3925), vec=x3906_vector) { implicit CU => 
      ...
      var stage: List[Stage] = Nil
      stage = stage0 +: Stages(1)
      Stage(stage(1), operands=List(CU.scalarIn(stage(0), x3909_scalar), CU.ctr(stage(0), x3927_cc(0))), op=FixAdd, results=List(CU.scalarOut(stage(1), x3927_mc_mc.saddr)))
    }
