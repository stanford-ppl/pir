...
object MatMult_inner extends PIRApp {
  ...
  def main(top:Top) = {
    ...
    val b_oc = OffChip("b")
    val a_addr_da = DRAMAddress("a_addr", "a")
    val x1910_x1929_s = Scalar("x1910_x1929")
    val c_addr_da = DRAMAddress("c_addr", "c")
    val a_oc = OffChip("a")
    ...
    val b_addr_da = DRAMAddress("b_addr", "b")
    ...
    val x1985 = MetaPipeline(name="x1985",parent=x1986) { implicit CU => 
      ...
      val x1816_x1835 = ScalarBuffer().wtPort(M_argin)
      val ctr1 = Counter(min=Const(0), max=x1816_x1835.load, step=Const(16), par=1) // Counter
      val ctr2 = Counter(min=Const(0), max=x1817_x1833.load, step=Const(16), par=1) // Counter
    ...
    val x1838_dsp0 = MemoryPipeline(name="x1838_dsp0",parent="x1985") { implicit CU => 
      ...
      val x1838_x1969 = SRAM(size=256,banking = Strided(1)).wtPort(x1941_x1941.readPort).rdPort(x1838_x1969_x1973_v)
      WAStage(operands=List(CU.ctr(x1909(0)), Const(16)), op=FixMul, results=List(b2033))
      WAStage(operands=List(b2033, CU.ctr(x1909(1))), op=FixAdd, results=List(x1838_x1969.writeAddr))
      RAStage(operands=List(CU.ctr(x1946(0)), Const(16)), op=FixMul, results=List(b2039))
    ...
    val x1838_dsp1 = MemoryPipeline(name="x1838_dsp1",parent="x1985") { implicit CU => 
      ...
      RAStage(operands=List(CU.ctr(x1909(0)), Const(16)), op=FixMul, results=List(b2031))
    ...
    val x1944 = MetaPipeline(name="x1944",parent=x1985) { implicit CU => 
      val x1818_x1839 = ScalarBuffer().wtPort(P_argin)
      val ctr3 = Counter(min=Const(0), max=x1818_x1839.load, step=Const(16), par=1) // Counter
    ...
    val x1842_dsp0 = MemoryPipeline(name="x1842_dsp0",parent="x1944") { implicit CU => 
      ...
      val x1842_x1918 = SRAM(size=256,banking = Strided(1)).wtPort(x1872_x1872.readPort).rdPort(x1842_x1918_x1931_v)
      WAStage(operands=List(CU.ctr(x1845(0)), Const(16)), op=FixMul, results=List(b2019))
      WAStage(operands=List(b2019, CU.ctr(x1865(0))), op=FixAdd, results=List(x1842_x1918.writeAddr))
      RAStage(operands=List(CU.ctr(x1909(0)), Const(16)), op=FixMul, results=List(b2027))
    ...
    val x1862_0 = Pipeline(name="x1862_0",parent=x1874) { implicit CU => 
      ...
      Stage(operands=List(Const(64)), op=Bypass, results=List(CU.scalarOut(x1846_b2016_x1861_b2018_s)))
    }
    val x1863 = MemoryController(name="x1863",parent=x1874,offchip=a_oc, mctpe=TileLoad) { implicit CU => 
    ...
    val x1873 = Pipeline(name="x1873",parent=x1874) { implicit CU => 
      ...
      val x1865 = CounterChain(name = "x1865", ctr5).iter(1)
    }
    ...
    val x1893_0 = Pipeline(name="x1893_0",parent=x1905) { implicit CU => 
      ...
      Stage(operands=List(Const(64)), op=Bypass, results=List(CU.scalarOut(x1877_b2022_x1892_b2024_s)))
    }
    val x1894 = MemoryController(name="x1894",parent=x1905,offchip=b_oc, mctpe=TileLoad) { implicit CU => 
    ...
    val x1904 = Pipeline(name="x1904",parent=x1905) { implicit CU => 
      ...
      val x1896 = CounterChain(name = "x1896", ctr7).iter(1)
    }
    val x1943 = MetaPipeline(name="x1943",parent=x1944) { implicit CU => 
      ...
      val x1909 = CounterChain(name = "x1909", ctr8, ctr9).iter(256)
    }
    val x1931_0 = Pipeline(name="x1931_0",parent=x1943) { implicit CU => 
      ...
      val x1912 = CounterChain(name = "x1912", ctr10).iter(1)
    ...
    val x1963_0 = Pipeline(name="x1963_0",parent=x1984) { implicit CU => 
      ...
      Stage(operands=List(Const(64)), op=Bypass, results=List(CU.scalarOut(x1947_b2036_x1962_b2038_s)))
    }
    val x1973 = Pipeline(name="x1973",parent=x1984) { implicit CU => 
      val ctr12 = Counter(min=Const(0), max=Const(16), step=Const(1), par=16) // Counter
      val x1965 = CounterChain(name = "x1965", ctr12).iter(1)
    }
    val x1975 = MemoryController(name="x1975",parent=x1984,offchip=c_oc, mctpe=TileStore) { implicit CU => 
