...
object PageRank extends PIRApp {
  def main(args: String*)(top:Top) = {
    ...
    val x4053_x4123_x4144_v = Vector("x4053_x4123_x4144")
    ...
    val x4074_0 = Pipeline(name="x4074_0",parent=x4154) { implicit CU => 
      ...
      val x4065 = CU.temp
      val x4066 = CU.temp
      val ar186 =  ScalarFIFO(size=1).wtPort(bus_186_s)
      ...
      Stage(operands=List(x4067, Const(0), Const(1)), op=Mux, results=List())
    }
    ...
    val x4144 = StreamController(name="x4144",parent=x4154) { implicit CU => 
      val x3909_x4116 =  ScalarBuffer().wtPort(x3909_x3918_s)
      val ctr13 = Counter(min=Const(0), max=x3909_x4116.load, step=Const(1), par=1) // Counter
    ...
    val x4144_1 = Pipeline(name="x4144_1",parent=x4144) { implicit CU => 
      ...
      val x4123_x4123 =  VectorFIFO(size=1).wtPort(x4053_x4123_x4144_v)
