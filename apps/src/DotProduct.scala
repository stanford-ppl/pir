...
object DotProduct extends PIRApp {
  def main(args: String*)(top:Top) = {
    ...
    val x786 = Pipeline(name="x786",parent=x794) { implicit CU => 
      val x776_x785 =  ScalarFIFO(size = 16).wtPort(x776_x782_s)
      val ctr6 = Counter(min=Const(1), max=Const(1), step=Const(1), par=1) // Counter
      val x786_unit = CounterChain(name = "x786_unit", ctr6)
      var stage: List[Stage] = Nil
    }
    ...
    val x807 = Pipeline(name="x807",parent=x815) { implicit CU => 
      val x797_x806 =  ScalarFIFO(size = 16).wtPort(x797_x803_s)
      val ctr11 = Counter(min=Const(1), max=Const(1), step=Const(1), par=1) // Counter
      val x807_unit = CounterChain(name = "x807_unit", ctr11)
      var stage: List[Stage] = Nil
    }
    ...
    val x834 = Pipeline(name="x834",parent=x835) { implicit CU => 
      ...
      Stage(stage(2), operands=List(rr126), op=Bypass, results=List(CU.scalarOut(stage(2), x769_x833_s)))
    }
    val x838 = Pipeline(name="x838",parent=x839) { implicit CU => 
      val x769_x836 =  ScalarBuffer().wtPort(x769_x833_s)
      val ctr15 = Counter(min=Const(1), max=Const(1), step=Const(1), par=1) // Counter
      val x838_unit = CounterChain(name = "x838_unit", ctr15)
      var stage: List[Stage] = Nil
      stage = CU.emptyStage +: Stages(1)
