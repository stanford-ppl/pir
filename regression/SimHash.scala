...
@spatial class SimHash(K:scala.Int = 5, L:scala.Int = 3, ratio:scala.Int = 3) extends SpatialTest {
  ...
  def main(args: Array[String]) : Unit = {
    val dim_0 = 100
    val dim_1 = 50
    val vector = loadCSV2D[T](sys.env("SPATIAL_HOME") + "/test-data/sim_hash/gold_input.csv", ",")
    writeCSV2D(vector, sys.env("SPATIAL_HOME") + "/test-data/sim_hash/spatial_input.csv", ",")
    ...
    writeCSV2D(out, sys.env("SPATIAL_HOME") + "/test-data/sim_hash/spatial_out.csv", ",")
    val gold = loadCSV2D[Int](sys.env("SPATIAL_HOME") + "/test-data/sim_hash/gold_output.csv", ",")
