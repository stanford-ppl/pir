...
//     beta1:scala.Float = 0.99,
//     beta2:scala.Float = 0.999,
//     eps:scala.Float = 1e-9f,
...
//     def backprop(active_len_in: Int, counter_in: Int, counter_out: Int, lr: Float, w: SRAM2[T], mw: SRAM2[T], vw: SRAM2[T], b: SRAM1[T], mb: SRAM1[T], vb: SRAM1[T], err_out: SRAM1[T], h_in: SRAM1[T], h_out: SRAM1[T], iter: Int) = {
...
//         Foreach(0 until counter_out by 1) { i =>
//             Foreach(0 until counter_in by 1 par ip) { j =>
//                 val g = Reg[T](err_out(i) * h_in(j) * mux(h_out(i) >= 0, 1.to[T], 0))
//                 mw(i, j) = beta1 * mw(i, j) + (1 - beta1) * g
//                 vw(i, j) = beta2 * vw(i, j) + (1 - beta2) * g * g
//                 val mw_hat = mw(i, j) / (1 - beta1 ** iter)
//                 val vw_hat = vw(i, j) / (1 - beta2 ** iter)
//                 w(i, j) = w(i, j) - lr * mw_hat / sqrt(vw_hat + eps)
...
//             val g = Reg[T](err_out(i) * mux(h_out(i) >= 0, 1.to[T], 0))
//             mb(i) = beta1 * mb(i) + (1 - beta1) * g
//             vb(i) = beta2 * vb(i) + (1 - beta2) * g * g
//             val mb_hat = mb(i) / (1 - beta1 ** iter)
//             val vb_hat = vb(i) / (1 - beta2 ** iter)
//             b(i) = b(i) - lr * mb_hat / sqrt(vb_hat + eps)
...
//         // momentum for adam
//         val d_mw_l1 = DRAM[T](L1 * field)
//         val d_mw_l2 = DRAM[T](L2 * L1)
//         val d_mb_l1 = DRAM[T](L1)
//         val d_mb_l2 = DRAM[T](L2)
//         // velocity for adam
//         val d_vw_l1 = DRAM[T](L1 * field)
//         val d_vw_l2 = DRAM[T](L2 * L1)
//         val d_vb_l1 = DRAM[T](L1)
//         val d_vb_l2 = DRAM[T](L2)
